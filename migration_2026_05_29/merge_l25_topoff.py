...
The historical per-refresh L25 deltas were deleted in the 2026-05-27 single-source
dedup, so consolidate_l25_to_canonical.py can no longer rebuild from sources. Instead
we stream [existing canonical {asset}.parquet, new refresh_2026_05_29 delta] through
the same max_seen dedup + ParquetWriter (row_group_size=200_000, avoids the BTC
truncation bug) into a temp file, then atomically replace the canonical. Preserves all
history (Apr 22 -> now) in one file per asset.
"""
...
DELTA_DIR = DATA / "refresh_2026_05_29" / "cache"
...
def merge_asset(asset):
    ...
    if tmp.exists():
        tmp.unlink()
    # stream existing canonical FIRST (chronological), then delta. dedup by max ts per (slug,outcome).

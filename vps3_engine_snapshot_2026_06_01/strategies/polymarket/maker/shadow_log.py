...
SHADOW_LOG_COLUMNS: list[str] = [
    ...
    "slug_offset_s",
    "trigger_reason",
    # Bug 8 (TV_AGENT guide): per-cell sleeve_id (e.g. "poly_acc_m_btc_5m_shadow")
    # so multi-cell runs sharing one strategy CSV can be filtered downstream.
    # Appended AFTER trigger_reason (the previous last column) per the
    # header docstring's "append-only" contract.
    "sleeve_id",
]
...
class AsyncShadowLogger:
    ...
    def _row_from_decision(
        ...
        if slug_state is not None:
            ...
            # Bug 2 (TV_AGENT guide): cash_recovered from MERGE + REDEEM.
            row["cash_recovered"] = str(slug_state.cash_recovered)
            # Bug 1 (TV_AGENT guide): per-row running PnL snapshot so
            # downstream tooling doesn't need to recompute from the cash
            # columns. Includes mark-to-market on paired residual since
            # those shares are guaranteed $1 at resolution per CTF binary
            # invariant. Unpaired residual marks at 50/50 (uncertain).
            #
            # 2026-05-22 Bug-9 mirror: once REDEEM has fired for this slug
            # (``slug_state.redeem_fired = True``, set by
            # ``poly_maker_fill_sim._observe_redeem``), residual is the
            # LOSER side — provably worth $0 (CTF pays $1 only to the
            # winning outcome). Marking dust at $0.50 here pre-fix made
            # mas · btc_15m show ~$0 PnL when each slug actually
            # overstated by $15. The LIST endpoint in
            # ``backend/app/api/maker_sleeves.py`` already had this fix;
            # bringing the engine in sync so card/drawer/CSV all agree.
            paired = min(slug_state.inv_up, slug_state.inv_dn)
            residual = abs(slug_state.inv_up - slug_state.inv_dn)
            if slug_state.redeem_fired:
                mark = paired * Decimal("1.0")
            else:
            ...
            # Bug 3 (TV_AGENT guide): elapsed seconds since slug_active.
            if slug_state.slug_active_ts_us > 0:
                offset_us = decision.ts_us - slug_state.slug_active_ts_us
                row["slug_offset_s"] = str(offset_us / 1_000_000)
        # Bug 8 (TV_AGENT guide): derive sleeve_id per-row from the decision
        # (a single logger may serve multiple cells of the same strategy —
        # e.g. ACC-M on btc_5m + btc_15m). Format matches
        # poly_maker_loop._sleeve_id: poly_<strategy>_<asset>_<tf>_shadow.
        if decision.strategy and decision.asset and decision.tf:
            row["sleeve_id"] = (
                f"poly_{decision.strategy.lower().replace('-', '_')}_"
                f"{decision.asset.lower()}_{decision.tf}_shadow"
            )
        return row

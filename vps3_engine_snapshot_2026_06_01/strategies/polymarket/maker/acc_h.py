...
class AccHStrategy(AccMStrategy):
    ...
    def _pair_cost_arb_ok(self, slug: str) -> bool:
        ...
        if up_evt is None or dn_evt is None:
            return False
        # E12 (ENGINE_BUG_MAP_2026_05_28): staleness guard — when fill-realism
        # is on, reject pair-cost fires evaluated against an L25 cache older
        # than 10s (trade-print bursts can fire against a stale book).
        if getattr(self.config, "tv_poly_maker_fill_realism", False) is True:
            import time as _time
            _now_us = int(_time.time() * 1_000_000)
            _max_age_us = 10 * 1_000_000
            if (_now_us - up_evt.ts_us) > _max_age_us or (
                _now_us - dn_evt.ts_us
            ) > _max_age_us:
                return False

...
class Vwap15mStore:
    ...
    def close_at(self, asset: str, ts_us: int) -> float | None:
        """Most-recent 1s close at-or-before ``ts_us`` (microseconds). None when
        no bar exists for ``asset`` or every bar is after ``ts_us``.

        This is the engine-side ``binance_1s`` price-at(ts) read the fast_taker
        intra-window-return signal needs — px@slot_open and px@fire — matching
        the backtest's ``binance_1s(slot_start)`` / ``binance_1s(fire)`` reads
        (TV_AGENT_FIX_LAGV2_SIGNAL_2026_06_01). Reverse-scan, O(window); the
        deque is bounded at ``max_bars`` (960 = 16 min, so a 15m slot_start is
        always in range).
        """
        d = self.bars.get(asset)
        if not d:

...
class VwapKellyFe1000Strategy(VwapKellyEnsembleStrategy):
    """V10 — de-risked Kelly variant (TV_AGENT_SPEC_V10_SLEEVES_2026_05_31.md §4).

    Clone of :class:`VwapKellyEnsembleStrategy` with three spec changes:

    1. **Conviction floor** — fire ONLY when ``|fair_edge_bp| > 1000`` (the
       parent fired on every positive-edge tier / S4∪S8). Direction is the
       *fair-value-rich* side: the leg with the larger ``fair_edge_bp``
       (= ``sign(fair_edge)``), not ``sign(dev_bps)``.
    2. **½-Kelly** — base unit ``$12.5`` (not ``$25``) × the same tier mult,
       so stake spans ~$25 (mult 2, fe just over 1000) to ~$50 (mult 4,
       fe > 3000) instead of ~$50–$100.
    3. **No ``keep_EU`` time-of-day gate** — it failed OOS (both-half
       holdout); ``fair_edge_bp>1000`` is the durable replacement. (TV's
       ensemble never carried ``keep_EU``, so this is a no-op on the port —
       documented for parity with the lab parent.)

    SHADOW/paper only; runs ALONGSIDE the parent for live OOS A/B.
    ``fair_edge_bp`` predictiveness decay is the documented kill trigger.
    """

    name = "vwap_kelly_fe1000"

    def __init__(
        self,
        *,
        fair_edge_floor_bp: float = 1000.0,
        base_notional_usd: Decimal = Decimal("12.5"),
    ) -> None:
        super().__init__(base_notional_usd=base_notional_usd)
        self.fair_edge_floor_bp = float(fair_edge_floor_bp)
        self.mode = "vwap_kelly_fe1000"
    ...
    def signal(
        ...
        if aux is None:
            return "NONE"
        # Same phase gate as the parent: t_plus_120 or later.
        phase = str(aux.get("bar_ctx_phase") or "")
        if not phase.startswith("t_plus_"):
            return "NONE"
        try:
        ...
        fe_up = _finite(aux.get("fair_edge_bp"))
        fe_dn = _finite(aux.get("fair_edge_bp_down"))
        if fe_up is None and fe_dn is None:
            return "NONE"

        # Direction = the fair-value-rich side (larger fair_edge_bp leg).
        if fe_dn is None or (fe_up is not None and fe_up >= fe_dn):
            direction: SignalResult = "UP"
            fe = fe_up
        else:
            direction = "DOWN"
            fe = fe_dn
        if fe is None or fe <= self.fair_edge_floor_bp:
            return "NONE"

        # ½-Kelly tier sizing via the controller override slot.
        mult = _kelly_mult_for_edge(fe)
        controller = aux.get("_controller_ref")
        if controller is not None:
            try:
                ...
                controller._kelly_rule_label = "fe1000"
                controller._kelly_mult = mult
            except Exception:  # pragma: no cover — defensive
                pass
        return direction

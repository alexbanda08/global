#!/bin/bash
# Complete L25 pull: May 6 14:00 -> May 17 00:00 UTC. Replaces 05_12 + 05_16 deltas.
# Slug-filter only (no mids JOIN) so we capture every market in the window.
set -euo pipefail
set -a; source /etc/tv/tv-ro.env; set +a
export PGPASSWORD="$TV_RO_PWD_PLAIN"

W_LO=1778421600000000   # 2026-05-06 14:00 UTC
W_HI=1778976000000000   # 2026-05-17 00:00 UTC

for ASSET in btc eth sol; do
  OUT=/tmp/${ASSET}_l25_complete_2026_05_16.csv
  echo "=== $ASSET ==="
  T0=$(date +%s)
...
echo
echo "=== summary ==="
ls -la /tmp/*_l25_complete_2026_05_16.csv.gz

"""
kronos_infer_v3_universe — run BTC fine-tuned Kronos on the BTC markets_minimal universe.

Inputs:
  data/v4/refresh_2026_05_02/btc_markets_minimal.csv (4,673 BTC markets, fresh today)
  strategy_lab/kronos_ft/data/BTCUSDT_5m_ext.csv     (5m OHLCV through May 4 UTC)

Output:
  strategy_lab/results/kronos/kronos_btc_predictions_full.csv

Behavior:
  - SAMPLE_COUNT defaults to 5 (~1.5h for 4,673 markets)
  - line-buffered stdout so background log shows live progress
  - Logs progress every 25 markets with ETA
  - Skips markets whose context bar is missing in the 5m data
  - Resumes if output CSV already exists (only runs on missing slugs)
"""
...
OUT_CSV = HERE / "results" / "kronos" / "kronos_btc_predictions_full.csv"
...
def main():
    ...
    print(f"V3 markets to score: {len(markets)} (pre-resume filter)", flush=True)

    done = load_existing_output()
    if done:
        print(f"Resume: {len(done)} slugs already in output, skipping", flush=True)
    ...
    failed = 0
    t0 = time.time()
    flush_every = 25  # write to disk in chunks for resume safety
    ...
    for k, m in markets.iterrows():
        ...
        if (k + 1) % flush_every == 0:
            ...
            print(f"  [{k+1}/{len(markets)}] el={el:.0f}s rate={rate:.2f}/s eta={eta/60:.1f}min "
                  f"skipped={skipped_missing_ctx} failed={failed}", flush=True)

    if rows:
        chunk = pd.DataFrame(rows)
        chunk.to_csv(OUT_CSV, mode="a", header=write_header, index=False)
    el = time.time() - t0
    print(f"\nDone in {el/60:.1f}min. skipped_missing_ctx={skipped_missing_ctx} failed={failed}", flush=True)

...
**2026-07-29 · TVRUST · vps_ireland · This is the centerpiece of the Windows page from `TV_AGENT_SPEC_STRAT_DASHBOARD_V1_2026_07_28.md`. Target look: Polymarket's 5m event chart (smooth real-time probability line) with OUR orders drawn on it. §R below = rulings on your 07-28/29 report — read first.**

## §R — Rulings on your last report (disk/partitioning/fee/A-B), then this spec is the next workstream
1. **Disk + partitioning — ACCEPTED.** Root-cause honesty appreciated; budget `target/` from now on (release-only builds on the box, or clean debug artifacts at session end — your call, write the policy into STATUS.md). Partition surgery verified by results (22.4s→5.6ms); in-repo re-runnable script is right.
2. **Retention — ruling:** automate your option (1) monthly compaction (same DETACH/ATTACH trick) + option (3) automation so the default partition never silently collects again. **Do NOT thin `ladder_tick` at source** — §1–2 of THIS spec depend on it for window replay. Instead: keep `ladder_tick`/`feed_quality` raw 30 days then drop via the monthly job; `tick_latency` MAY thin at source to per-minute p50/p95/p99 aggregates AFTER the 12h post-pin after-table is delivered. Compression dead-end accepted — thanks for saying so instead of proposing it.
3. **🔴 Fee sign-flip — ACCEPTED, no code/strategy changes.** Two additions: (a) record in the report that OUR paper fills pair at pvs ≈0.75–0.83, far below the 0.966 break-even — the pair leg is strongly EV-positive at our fill prices; the flip threatens near-1.0 paircosts (b945-style), not our ladder — so no strategy change is warranted. (b) Your caveat IS the deliverable: **"does redemption charge the taker rate" goes on the session-1 measurement list** — the first held pair that resolves settles it empirically, free. Keep `capture_ratio.py` reporting both `capture_today` and `capture_merged` until measured.
4. **A/B page — ACCEPTED.** Paired-per-window join and "not comparable ≠ inconclusive" are exactly right. UI must show sample-age loudly: c2rcg +0.915/w t=3.14 is YOUNG (n=178, started 07-27) — no promotion discussion until its pre-registered n≥2,000. Show BOTH horizons for mature arms (c2 7d +0.197 t=2.24 vs full-life +0.321 t=4.06 = regime wobble, not decay).
5. **Sequencing for this spec:** the chart REQUIRES `/ws/strat` push — polling cannot carry a moving chart — so build **WS push + this chart as ONE slice**. Tape/Health pages + PWA come after. Everything here is tv-api + frontend only — NO engine restart; the §5 `best_ask` addition rides the next scheduled restart only.
6. **Funding sequence unchanged:** operator deposits + approvals → you verify allowances on-chain → report → STOP.

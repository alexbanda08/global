"""Master 15m feature panel — same features as build_master_5m_panel.py
but pointed at the 15m per-fire input + 15m slot timestamps.

Run: py strategy_lab/overnight_2026_05_23/build_master_15m_panel.py
"""
...
PT_IN  = ROOT / "data" / "v4" / "canonical" / "_results" / "vwap_continuation_15m_per_fire.parquet"
KL1S   = ROOT / "data" / "v4" / "canonical" / "klines_1s" / "binance_1s_28d.parquet"
OUT    = ROOT / "data" / "v4" / "canonical" / "_results" / "master_15m_panel.parquet"
WINDOW_S = 900    # 15m


# Re-use builders identical to 5m
def build_1s_arrays(asset: str) -> dict:
    ...
    df = df.sort_values(["time_period_start_us", "source"]).drop_duplicates("time_period_start_us", keep="last").reset_index(drop=True)
...
def macd_at(arrs, t_us):
    ts=arrs["ts"]; i=int(np.searchsorted(ts,t_us,side="right"))-1
...
def sigma(arrs, t_us, win=900):
    ...
    r=np.diff(c); return float(np.std(r,ddof=1)) if len(r)>1 else float("nan")
...
def micro_at(books, slug, side, fire_us, lat=85_000, stale=60_000_000):
    ...
    ap0=float(ap[j,0]); bp0=float(bp[j,0]); az0=float(asz[j,0]); bz0=float(bsz[j,0])
    ...
    return {"mid":mid,"micro":micro,"micro_minus_mid_bp":(micro-mid)/mid*10_000,
            "spread_bp":spread_bp,"imb5":imb5,"best_bid":bp0,"best_ask":ap0}
...
def main():
    ...
    fires["slot_start_us"]=fires["ws_s"]*1_000_000
    fires["slot_end_us"]=fires["slot_start_us"]+WINDOW_S*1_000_000

    print(f"[2] per-asset data load...")
    ...
    for a,sub in fires.groupby("asset"):
        ...
        books_by_asset[a]=load_orderbook_l25_streaming(
            ...
            max_ts_us=int(sub["fire_us"].max())+3*WINDOW_S*1_000_000,
        )
    ...
    for idx, r in enumerate(fires.itertuples(index=False)):
        if idx%2000==0: print(f"   row {idx:,}/{len(fires):,}")
        a=r.asset
        oa_set=[x for x in ("BTC","ETH","SOL") if x!=a]
        fire_us=int(r.fire_us); slot_start_us=int(r.slot_start_us); slot_end_us=int(r.slot_end_us)
        ...
        sg=sigma(one_s[a],fire_us,900); sg5=sigma(one_s[a],fire_us,300)
        if (np.isfinite(strike) and np.isfinite(sn) and np.isfinite(sg) and sg>0 and tau_s>0):
            z=math.log(sn/strike)/(sg*math.sqrt(tau_s))
            fair_up=float(norm.cdf(z))
        else: fair_up=float("nan")
        ...
        # B. CVD slopes
        c30=cvd_slope(one_s[a],fire_us,30); c60=cvd_slope(one_s[a],fire_us,60); c120=cvd_slope(one_s[a],fire_us,120)
        ...
        # E. RVOL
        rv1=rvol(one_s[a],fire_us,30,300); rv2=rvol(one_s[a],fire_us,60,900)
        ...
        if mp is None:
            ...
            spread_bp=mp["spread_bp"]; imb5=mp["imb5"]; best_bid=mp["best_bid"]; best_ask=mp["best_ask"]
        ...
        m1v_pass=_p(m1v); m5v_pass=_p(m5v); m1f_pass=_p(m1f); m5f_pass=_p(m5f)
        ...
        rows.append({
            ...
            "s_now": sn, "vwap_15m": float(r.vwap_15m), "dev_bps": float(r.dev_bps),
            ...
            "spread_bp": spread_bp, "imb5": imb5, "best_bid": best_bid, "best_ask": best_ask,
            "m1v_pass": m1v_pass, "m5v_pass": m5v_pass, "m1f_pass": m1f_pass, "m5f_pass": m5f_pass,

"""Compute all 37 gates on the v2 master features dataframe."""
import os, time
import numpy as np
import pandas as pd
...
log("loading v2 master features")
df = pd.read_parquet(FP)
log(f"rows={len(df):,} cols={len(df.columns)}")

ds = df['dir_sign'].values
...
if 'vwap_since_open_bps' in df.columns:
    g = (df['vwap_since_open_bps'].abs() <= 50).astype(float)
    g = np.where(df['vwap_since_open_bps'].isna(), np.nan, g)
    df['g_within_dev'] = g
if 'vwap_since_open_bps' in df.columns:
    g = (df['vwap_since_open_bps'].abs() > 100).astype(float)
    g = np.where(df['vwap_since_open_bps'].isna(), np.nan, g)
    df['g_dev_extreme'] = g
...
if 'up_queue_top_bid' in df.columns:
    qmed = df['up_queue_top_bid'].median()
    g = (df['up_queue_top_bid'] > qmed).astype(float)
...
# coinbase basis: not present here as cross_a_dev_bp wasn't merged (came from master_5m_panel); compute via mlofi_skew sign
if 'mlofi_skew_l25_30s' in df.columns:
    a = df['mlofi_skew_l25_30s']
...
if 'hawkes_recent_burst' in df.columns:
    ...
    if 'hawkes_lambda_imbalance' in df.columns:
        g_with = np.where(ds > 0, df['hawkes_lambda_imbalance'] > 0, df['hawkes_lambda_imbalance'] < 0).astype(float)
        g = g_burst * g_with
    else:
        g = g_burst
    g = np.where(df['hawkes_recent_burst'].isna(), np.nan, g)
    df['g_hl_liq_cascade_with'] = g
# hy_cb_with_dir not avail (no coinbase basis col here)

gates = [c for c in df.columns if c.startswith('g_')]
log(f"computed {len(gates)} gates")
log("WR when TRUE:")
for g in sorted(gates):
    ...
    if len(sub) > 100:
        ...
        print(f"  {g:42s} cov={cov:5.1f}%  n={n:6d}  WR={wr:5.2f}%  $/tr={pt:+.3f}")

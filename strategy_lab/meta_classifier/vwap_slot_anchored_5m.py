"""Slot-anchored VWAP continuation — variant of S1 with cleaner anchor.

Difference from `vwap_continuation_5m.py`:
- S1: anchor = start of current 15m UTC bucket (resets every 15min on the
  :00/:15/:30/:45 marks).
- This: anchor = slot_open (the start of THIS 5m market's slot). Resets
  every 5min, exactly aligned with market windows.

Hypothesis: slot-anchored VWAP gives a cleaner "where has binance gone
since strike was set?" signal. Strike is set at slot_open (chainlink
RTDS sample), so VWAP from slot_open to fire_s shows how binance has
moved relative to the strike's reference.

Output:
  data/v4/canonical/_results/vwap_slot_anchored_5m.csv
  strategy_lab/reports/VWAP_SLOT_ANCHORED_5M_2026_05_23.md
"""
from __future__ import annotations

import math
import sys
from datetime import datetime, timezone
from pathlib import Path

import numpy as np
import pandas as pd

ROOT = Path(r"C:\Users\alexandre bandarra\Desktop\global")
sys.path.insert(0, str(ROOT / "data" / "v4" / "canonical"))
sys.path.insert(0, str(ROOT / "strategy_lab"))

from load import load_resolutions, load_orderbook_l25_streaming  # noqa: E402
from engine_v2 import LegacyConfig, fill_at_book, hold_pnl  # noqa: E402

KL1S = ROOT / "data" / "v4" / "canonical" / "klines_1s" / "binance_1s_28d.parquet"
OUT_CSV = ROOT / "data" / "v4" / "canonical" / "_results" / "vwap_slot_anchored_5m.csv"
OUT_PT = ROOT / "data" / "v4" / "canonical" / "_results" / "vwap_slot_anchored_5m_per_fire.parquet"
OUT_MD = ROOT / "strategy_lab" / "reports" / "VWAP_SLOT_ANCHORED_5M_2026_05_23.md"

FIRE_OFFSETS_S = (30, 60, 90, 120, 150, 180, 210, 240, 270)
SPREAD_FILTER = {"BTC": 0.02, "ETH": 0.02, "SOL": 0.025}
SYM_MAP = {
    "BINANCE_SPOT_BTC_USDT": "BTC",
    "BINANCE_SPOT_ETH_USDT": "ETH",
    "BINANCE_SPOT_SOL_USDT": "SOL",
}


def asof_idx(ts, target_us):
    i = int(np.searchsorted(ts, target_us, side="right")) - 1
    if i < 0 or i >= len(ts):
        return -1
    return i


def build_per_asset(df_1s):
    df_1s = df_1s.sort_values(["symbol_id", "time_period_start_us", "source"])
    df_1s = df_1s.drop_duplicates(["symbol_id", "time_period_start_us"], keep="last")
    df_1s["asset"] = df_1s["symbol_id"].map(SYM_MAP)
    out = {}
    for asset in ("BTC", "ETH", "SOL"):
        sub = df_1s[df_1s["asset"] == asset].copy().sort_values("time_period_start_us").reset_index(drop=True)
        out[asset] = {
            "ts_us": sub["time_period_start_us"].values.astype("int64"),
            "close": sub["price_close"].values.astype("float64"),
            "vol": sub["volume_traded"].fillna(0).values.astype("float64"),
        }
        print(f"  {asset}: 1s rows={len(out[asset]['ts_us']):,}")
    return out


def slot_vwap(asset_arrs, slot_start_us: int, fire_us: int) -> tuple[float, float]:
    """VWAP from slot_start_us to fire_us. Returns (vwap, close_now). NaN if not computable."""
    ts = asset_arrs["ts_us"]
    cl = asset_arrs["close"]
    vol = asset_arrs["vol"]
    lo = int(np.searchsorted(ts, slot_start_us, side="left"))
    hi = int(np.searchsorted(ts, fire_us, side="right"))
    if hi <= lo:
        return float("nan"), float("nan")
    cum_pv = float((cl[lo:hi] * vol[lo:hi]).sum())
    cum_v = float(vol[lo:hi].sum())
    if cum_v <= 0:
        return float("nan"), float("nan")
    vwap = cum_pv / cum_v
    # close at fire_us
    fi = hi - 1
    if fi < 0:
        return vwap, float("nan")
    return vwap, float(cl[fi])


def main():
    print("[1] loading 1s binance...")
    df_1s = pd.read_parquet(KL1S)
    print(f"    {len(df_1s):,} rows")

    print("[2] per-asset arrays...")
    arrs = build_per_asset(df_1s)
    min_ts = min(arrs[a]["ts_us"][0] for a in arrs)
    max_ts = max(arrs[a]["ts_us"][-1] for a in arrs)

    print("[3] loading 5m chainlink resolutions...")
    res = load_resolutions()
    res = res.rename(columns={"ticker": "asset", "timeframe": "tf"})
    res = res[(res.asset.isin(("BTC", "ETH", "SOL"))) & (res.tf == "5m")].copy()
    res = res[(res.slot_start_us >= min_ts) & (res.slot_start_us <= max_ts)].copy()
    res["slot_start_s"] = (res.slot_start_us // 1_000_000).astype("int64")
    res["slot_end_s"] = (res.slot_end_us // 1_000_000).astype("int64")
    print(f"    {len(res):,} 5m slugs in window")

    print("[4] generating candidates per (slug, offset)...")
    rows = []
    for asset in ("BTC", "ETH", "SOL"):
        sub = res[res.asset == asset]
        a = arrs[asset]
        for _, r in sub.iterrows():
            slot_start_s = int(r.slot_start_s)
            slot_end_s = int(r.slot_end_s)
            outcome = str(r.outcome)
            strike = float(r.strike_price)
            slug = r.slug
            for off in FIRE_OFFSETS_S:
                fire_s = slot_start_s + off
                if fire_s >= slot_end_s:
                    continue
                vwap_slot, close_now = slot_vwap(a, slot_start_s * 1_000_000, fire_s * 1_000_000)
                if not (math.isfinite(vwap_slot) and math.isfinite(close_now) and vwap_slot > 0):
                    continue
                dev_bps_vwap = 10000.0 * math.log(close_now / vwap_slot)
                # Also: dev relative to strike (more semantic)
                dev_bps_strike = 10000.0 * math.log(close_now / strike) if strike > 0 else float("nan")
                direction = "UP" if dev_bps_vwap > 0 else "DOWN"
                rows.append((slug, asset, fire_s, fire_s * 1_000_000, off,
                             close_now, vwap_slot, dev_bps_vwap, dev_bps_strike,
                             strike, direction, outcome, slot_end_s - fire_s))
    cands = pd.DataFrame(rows, columns=[
        "slug", "asset", "fire_s", "fire_us", "fire_offset_s",
        "close_now", "vwap_slot", "dev_bps_vwap", "dev_bps_strike",
        "strike", "direction", "outcome", "tau_sec",
    ])
    cands = cands[cands.dev_bps_vwap.abs() > 3].copy()
    print(f"    {len(cands):,} candidates after |dev_vwap|>3bps floor")

    print("[5] loading L25 books per asset...")
    books_idx = {}
    for asset in ("BTC", "ETH", "SOL"):
        slugs_a = set(cands[cands.asset == asset]["slug"].unique())
        if not slugs_a:
            continue
        print(f"    {asset}: {len(slugs_a)} slugs...")
        try:
            ab = load_orderbook_l25_streaming(asset, slugs=slugs_a, subsample_1hz=True)
            books_idx.update(ab)
            print(f"      loaded {len(ab)} keys")
        except Exception as e:
            print(f"      ERROR: {e}")

    print("[6] filling via engine_v2.LegacyConfig...")
    cfg = LegacyConfig()
    pnl = np.full(len(cands), np.nan)
    won = np.zeros(len(cands), dtype=bool)
    entry = np.full(len(cands), np.nan)
    cands = cands.reset_index(drop=True)
    for i, r in cands.iterrows():
        outcome_to_fill = "Up" if r.direction == "UP" else "Down"
        try:
            f = fill_at_book(books_idx, r.slug, outcome_to_fill, int(r.fire_us),
                             cfg=cfg, spread_filter=SPREAD_FILTER[r.asset])
        except Exception:
            f = None
        if f is None:
            continue
        w = (r.outcome == "Up") if r.direction == "UP" else (r.outcome == "Down")
        pnl[i] = hold_pnl(f, won=w, cfg=cfg)
        won[i] = w
        entry[i] = float(f["vwap"])
    cands["entry_vwap"] = entry
    cands["pnl_legacy_usd"] = pnl
    cands["won"] = won
    filled = cands[cands.pnl_legacy_usd.notna()].copy()
    print(f"    {len(filled):,} / {len(cands):,} fills successful")
    OUT_PT.parent.mkdir(parents=True, exist_ok=True)
    filled.to_parquet(OUT_PT, index=False, compression="zstd")

    # Aggregate
    bins = [3, 5, 10, 15, 20, 30, 50, 1e9]
    labels = ["3-5bps", "5-10bps", "10-15bps", "15-20bps", "20-30bps", "30-50bps", ">50bps"]
    filled["dev_tier"] = pd.cut(filled.dev_bps_vwap.abs(), bins=bins, labels=labels)
    g = filled.groupby(["asset", "fire_offset_s", "dev_tier"], observed=True).agg(
        n=("won", "count"), wr=("won", "mean"),
        avg_pnl=("pnl_legacy_usd", "mean"),
        sum_pnl=("pnl_legacy_usd", "sum"),
        avg_entry=("entry_vwap", "mean"),
    ).round(3).reset_index()
    g.to_csv(OUT_CSV, index=False)
    print(f"\n--- DEPLOYABLE (n>=30, WR>=60%, avg_pnl>0) ---")
    dep = g[(g.n >= 30) & (g.wr >= 0.60) & (g.avg_pnl > 0)].sort_values("sum_pnl", ascending=False)
    print(f"n_deployable: {len(dep)}")
    print(dep.head(40).to_string(index=False))

    print(f"\n--- ULTRA (n>=100, WR>=70%, $/tr>=$1) ---")
    ultra = g[(g.n >= 100) & (g.wr >= 0.70) & (g.avg_pnl >= 1.0)].sort_values("sum_pnl", ascending=False)
    print(f"n_ultra: {len(ultra)}")
    print(ultra.head(20).to_string(index=False))

    # Compare to 15m-anchored (S1) — top configs side by side
    print(f"\n--- COMPARISON to 15m-anchored S1 (top 5 sums by cell × offset × dev tier) ---")
    s1 = pd.read_csv(ROOT / "data/v4/canonical/_results/vwap_continuation_5m.csv")
    s1_dep = s1[(s1.n >= 30) & (s1.wr >= 0.60) & (s1.avg_pnl > 0)].sort_values("sum_pnl", ascending=False).head(15)
    print("S1 (15m-anchored) top 15:")
    print(s1_dep[["asset","fire_offset_s","dev_tier","n","wr","avg_pnl","sum_pnl"]].to_string(index=False))

    # Markdown
    md = []
    md.append(f"# VWAP slot-anchored — 5m markets ({datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')})")
    md.append("")
    md.append("Variant of S1 with anchor = slot_open (not 15m UTC bucket). Slot-anchored VWAP shows where binance has moved since the chainlink strike was set 0-300s ago.")
    md.append("")
    md.append("## Deployable (n>=30, WR>=60%, $/tr>0)")
    md.append("")
    if len(dep) > 0:
        md.append(dep.head(40).to_markdown(index=False))
    else:
        md.append("NONE")
    md.append("")
    md.append("## Ultra-strict (n>=100, WR>=70%, $/tr>=$1)")
    md.append("")
    if len(ultra) > 0:
        md.append(ultra.head(20).to_markdown(index=False))
    else:
        md.append("NONE")
    md.append("")
    md.append("## vs S1 (15m-anchored)")
    md.append("")
    md.append(s1_dep[["asset", "fire_offset_s", "dev_tier", "n", "wr", "avg_pnl", "sum_pnl"]].to_markdown(index=False))
    OUT_MD.write_text("\n".join(md), encoding="utf-8")
    print(f"\nReport: {OUT_MD}")
    return 0


if __name__ == "__main__":
    sys.exit(main())

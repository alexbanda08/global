...
async def _emit_state_tick(
    *,
    ...
    Note: notify_event is called once per ACTIVE slug per (strategy, cell).
    A cell with zero active slugs emits nothing (no sleeve to push).
    """
    for cell_key in cell_keys:
        sc = StratCell(strategy=cell_key[0], cell=cell_key[1])
        if sc in kill_set or sc in _disabled:
...
async def poly_maker_loop(
    *,
    ...
    for strat_code_lower, cells in cell_assignments.items():

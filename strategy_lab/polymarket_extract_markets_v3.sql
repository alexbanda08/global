-- V3: drive from market_resolutions_v2 (canonical outcomes + strike/settle).
-- Keeps the same column schema as v2 plus 3 new ones: strike_price, settlement_price, abs_move_pct.
...
WITH resolved AS (
  ...
  WHERE r.slug LIKE 'btc-updown-%'
    AND r.outcome IS NOT NULL
),
-- First in-window snap per (slug, outcome) — entry quote
entry_snap AS (
...
-- Last in-window snap per (slug, outcome) — final quote
last_snap AS (
  SELECT DISTINCT ON (r.slug, o.outcome)
    r.slug, o.outcome,
    o.bid_price_0 AS bid, o.ask_price_0 AS ask
...
CREATE INDEX ON tmp_btc_markets (slug);
ANALYZE tmp_btc_markets;
...
SELECT timeframe,
  ...
  ROUND(AVG(abs_move_pct * 100)::numeric, 4) AS avg_abs_move_pct,
  ROUND(AVG(n_snaps_yes)::numeric, 0) AS avg_snaps_per_market
FROM tmp_btc_markets
GROUP BY timeframe;

-- Export rows that have usable entry quotes
\copy (SELECT * FROM tmp_btc_markets WHERE entry_yes_ask IS NOT NULL AND entry_no_ask IS NOT NULL) TO '/tmp/btc_markets_v3.csv' WITH CSV HEADER;
\echo Exported markets_v3 to /tmp/btc_markets_v3.csv

"""V27 walk-forward OOS — same split and verdict as V25/V26."""
...
from strategy_lab.run_v27_swing import (
    _load, dedupe, scaled,
    sig_trend_pullback, sig_htf_donchian, sig_vwap_fade, sig_daily_ema_cross,
)

RES = Path(__file__).resolve().parent / "results" / "v27"
FEE = 0.00045
...
def _sig_for(d, df):
    ...
    if fam == "Trend_Pullback":
        return sig_trend_pullback(df, "4h", p["htf_ema"], p["ltf_ema"],
                                   14, p["rsi_lo"], p["rsi_hi"])
    if fam == "HTF_Donchian":
        return sig_htf_donchian(df, p["donch_n"], p["ema_reg"])
    if fam == "VWAP_Fade":
        return sig_vwap_fade(df, p["std_n"], p["dev_k"], scaled(400, d["tf"]))
    if fam == "Daily_EMA_X":
        return sig_daily_ema_cross(df, p["fast"], p["slow"], p["ema_reg"])
    raise ValueError(fam)


def main():
    path = RES / "v27_swing_results.pkl"
    if not path.exists():
        print("no V27 pickle")
        return 1
    ...
    df_out.to_csv(RES / "v27_oos_summary.csv", index=False)

    print("\n" + "=" * 80)
    print("V27 OOS VERDICTS")
    print("=" * 80)

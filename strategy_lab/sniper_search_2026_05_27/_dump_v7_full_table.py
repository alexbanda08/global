"""Dump complete V7 per-sleeve metrics across all 6 markets — unified table."""
import pandas as pd
import os
...
markets = [
    ('BTC 5m',  'btc_5m_v7/top_5_candidates_v7.csv'),
    ('ETH 5m',  'eth_5m_v7/_results/top_5_candidates_v7.csv'),
    ('SOL 5m',  'sol_5m_v7/top_5_candidates_v7.csv'),
    ('BTC 15m', 'btc_15m_v7/top_5_candidates_v7.csv'),
    ('ETH 15m', 'eth_15m_v7/top_5_candidates_v7.csv'),
    ('SOL 15m', 'sol_15m_v7/top_5_candidates_v7.csv'),
]
...
print('=== V7 master dump ===\n')

# Print every CSV first to understand schemas, then build unified
for name, rel in markets:
    p = f'{base}/{rel}'
    if not os.path.exists(p):
        print(f'### {name}: MISSING {p}\n'); continue
    df = pd.read_csv(p)
    print(f'### {name}  rows={len(df)}  source={rel}')
    cols = list(df.columns)
    print(f'    columns: {cols}\n')

print('\n=== Per-sleeve dump (best-effort schema mapping) ===\n')
...
for name, rel in markets:
    ...
    for i, r in df.iterrows():
        ...
        dpt_lb  = get(r, 'dpt_25_lockbox', 'dpt_25_lockbox_const', 'dpt_lockbox', 'dpt_lb')
        ...
        bs_p = get(r, 'bootstrap_p_lockbox', 'boot_p', 'p', 'bs_p_lockbox')

        sum28 = get(r, 'sum_25_28d_const', 'sum_25_lb_const', 'sum_25_const_lockbox', 'sum_25_lockbox_const', 'sum_28d', 'sum_lockbox')
        sum_lb = get(r, 'sum_25_lockbox', 'sum_lockbox', 'lockbox_sum')
        ...
        # Stability flag: any train/val negative dpt?
        try:
            stab_flag = '⚠VAL_NEG' if (float(dpt_v) < 0 if not pd.isna(dpt_v) else False) or \
                                       (float(dpt_tr) < 0 if not pd.isna(dpt_tr) else False) else 'OK'
        except: stab_flag = 'OK'

        print(f'  [{i+1}] {sid}  [{stab_flag}]')
        print(f'      gates : {gates[:140]}')
        ...
        print(f'      DD_lock=${fmt(dd_lb,"abs_dollar")}  LS_lock={fmt(ls_lb,"num",0)}  Sharpe={fmt(sharpe,"num",1)}  bs_p={fmt(bs_p,"p")}')
        print(f'      28d const PnL: ${fmt(sum28,"num",0)}    sum_lockbox: ${fmt(sum_lb,"num",0)}')

        rows_out.append({
            'market': name, 'rank': i+1, 'sleeve_id': sid, 'gates': gates,
            'anchor': str(anchor), 'dir': direction, 'stability': stab_flag,
            ...
            'sum_28d_const': sum28, 'sum_lockbox': sum_lb,
        })

# Save unified
out = pd.DataFrame(rows_out)
out.to_csv(f'{base}/_v7_full_table.csv', index=False)
print(f'\n\nSaved: {base}/_v7_full_table.csv ({len(out)} rows)')

"""
OPEN-SCALP KNOB RE-OPTIMIZATION — CORRECTED EXIT MODEL (2026-06-10). Fixed clone of scalp_entry_opt_2026_06_09.py.
Exit via scalp_fill_lib_2026_06_10.exit_fill (size-capped bid, staleness guard, no outcome-leak; remainder held).
Every cell reports ALL + CLEAN (frac_held==0). Env: EO_COINS/EO_TFS/EO_TAG/EO_LIMIT.
"""
...
WIN = ("2026-03-30", "2026-04-21"); TAG = os.environ.get("EO_TAG", ""); LIMIT = int(os.environ.get("EO_LIMIT", "0"))
        ...
        for i in range(0, len(slugs), B):
            ...
            for _, r in chunk.iterrows():
                ...
                for off in OFFS:
                    ...
                    ROWS.append(dict(coin=coin, tf=tf, slug=r.slug, off=off, delta=abs(delta), ev=ev,
                                     won=won, pnl60=ef["pnl"], frac_held=ef["frac_held"], held_full=ef["held_full"]))
            del bbo, bk
        F = pd.DataFrame(ROWS); F.to_parquet(RES / f"scalp_entryopt_fixed_2026_06_10{TAG}.parquet")
...
def dual(sub):
    return f"ALL {stat(sub.pnl60.values)} | CLEAN {stat(sub[sub.frac_held == 0].pnl60.values)}"


print("\n===== (1) ENTRY-OFFSET PLATEAU (cheap, |delta|>=3, exit +60), pooled =====")
for off in OFFS:
    print(f"  +{off:3d}s  {dual(G[G.off == off])}")
print("\n===== (1b) per coin-tf, +3 vs +5 vs +8 =====")
for (coin, tf), g in G.groupby(["coin", "tf"]):
    print(f"  {coin} {tf}: +3 {dual(g[g.off == 3])}")
    print(f"  {'':9s} +5 {dual(g[g.off == 5])}")
    print(f"  {'':9s} +8 {dual(g[g.off == 8])}")
...
for lo, hi in [(3, 999), (3, 12), (4, 12), (5, 12), (5, 999), (4, 10), (3, 8), (8, 999), (5, 15)]:
    ...
    print(f"  delta in [{lo:3d},{hi:3d}]  {dual(s)}")

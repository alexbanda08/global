...
from backend.app.data.lazer import get_lazer_instance
...
class PolymarketSniperV5Controller:
    ...
    def __init__(
        ...
        *,
        ...
        # resting-maker-SELL state {(sleeve_id, slug): {maker_price, order_id, mode}}.
    ...
    async def eval_sleeve_fire(
        ...
        needs_oracle = any(
            n.startswith((
                "g_oracle_lag_bps_ge",
                "g_oracle_lag_with",
                "g_lag_momentum_align",  # momalign reuses the oracle_lag snapshot
            ))
            for n in gate_names
        )
        ...
        # TV_AGENT_SPEC_SCALP_LAZER_DELTA_AB_2026_06_12 — δ source routing.
        # "lazer" arms take px@fire from the Pyth Lazer mirror (strike still =
        # binance@slot_start); a stale mirror → SKIP (no rtds fallback). The
        # binance δ at the same fire instant is logged as ``delta_rtds_at_fire``
        # for the offline disagreement analysis. Default "rtds" = unchanged.
        _delta_source = getattr(sleeve, "delta_source", "rtds")
        _lazer_skip = False
        if needs_oracle or needs_confluence:
            try:
                if _delta_source == "lazer":
                    snap = _lazer_lag_snapshot(
                        sleeve.asset, slot.slot_start_us, fire_us,
                    )
                    if snap is _LAZER_STALE:
                        _lazer_skip = True
                        ft_oracle_lag = None
                    else:
                        ft_oracle_lag = snap
    ...
    def _eval_sibling_confirms(
        ...
        *,
        ...
        """Evaluate a SIBLING sleeve's full gate-stack for the SAME slot+fire_us.

        Backs ``g_sleeve_confirms``. Re-runs the sibling's gates over the SAME
        per-cycle panels (via the identical ``_build_gate_kwargs``) — no re-fetch.
        require_same_direction True → sibling must pass on the candidate direction;
        False → on ANY of its directions. Unknown sibling → False (fail-closed).
        Nested ``g_sleeve_confirms`` gates in the sibling are SKIPPED (no recursion).
        """
        sib = SNIPER_V5_SLEEVE_BY_ID.get(sibling_id)
        if sib is None:
            return False
        dirs = (
            (candidate_direction,)
            if require_same_direction
            else self._directions_for(sib.direction)
        )
        for d in dirs:
            all_pass = True
            for gate_ref in sib.gates:
                if gate_ref.name.startswith("g_sleeve_confirms"):
                    continue
                runtime_kwargs = self._build_gate_kwargs(
                    gate_ref, sib, slot,
                    oracle_lag=oracle_lag, oracle_lag_other=oracle_lag_other,
                )
                static_kwargs = gate_ref.bound_kwargs()
                if not bool(gate_ref.gate(
                    d, fire_us, **runtime_kwargs, **static_kwargs,
                )):
                    all_pass = False
                    break
            if all_pass:
    ...
    def _book_dense_enough(
        self, token_id: str, fire_us: int,
        window_s: float = _BOOK_EVENT_WINDOW_S,
    ) -> bool:
        """Returns True if ≥ N book events landed in the last ``window_s`` sec.

        Note: ``fire_us`` is the schedule-time anchor but the actual call
        is wall-clock at-or-slightly-after fire_us. We anchor the window
        against the latest of (fire_us, now()) so live-fire reads the most
        recent ``window_s`` of book activity, and offline replay reads the
        window ending at fire_us. ``window_s`` defaults to the 60s roster
        default; DISAGR-HAWKES passes 120s (its validated gate).
        """
        threshold = int(
        ...
        cutoff_us = anchor_us - int(window_s * 1_000_000)
    ...
    def _build_gate_kwargs(
        ...
        *, oracle_lag: Any = None, oracle_lag_other: Any = None,
        ...
        if name.startswith("g_oracle_lag_with"):
            return {"oracle_lag": oracle_lag}
        # Momentum-alignment scalp gate
        # (TV_AGENT_SPEC_SCALP_MOMALIGN_BTC5M_2026_06_09) — reads the SAME
        # oracle_lag snapshot as g_oracle_lag_with PLUS the 1s binance store, so
        # the gate (which has fire_us) computes the trailing mom_win_s return
        # itself (mirrors g_disagr_hawkes' vwap_store injection). asset/mom_win_s
        # arrive as GateRef static kwargs.
        if name.startswith("g_lag_momentum_align"):
            _feed = get_feed_instance()
            return {
                "oracle_lag": oracle_lag,
                "vwap_store": getattr(_feed, "vwap_store", None) if _feed else None,
            }
        # DISAGR-HAWKES (TV_AGENT_SPEC_SHADOW_DISAGR_HAWKES_SOL5M) — the gate
        # reads both token books (mp_skew/imb5_diff) + computes the order-flow
        # Hawkes imbalance itself from the vwap_store 1s flow window (it has
        # fire_us; _build_gate_kwargs does not).
        if name.startswith("g_disagr_hawkes"):
            _feed = get_feed_instance()
            return {
        ...
        # Cross-sleeve AND-confirmation (TV_AGENT_SPEC_ETH_MERGE_HURST_2026_06_08).
        if name.startswith("g_sleeve_confirms"):
            def _confirm(
                sib_id: str, cand_dir: str, f_us: int, same_dir: bool,
            ) -> bool:
                return self._eval_sibling_confirms(
                    sib_id, cand_dir, slot, f_us, same_dir,
                    oracle_lag=oracle_lag, oracle_lag_other=oracle_lag_other,
                )
            return {"sibling_confirm_fn": _confirm}
        ...
        # vol_hurst_panel gates — g_vol_high AND the bottom-half vol_low trio.
        # TV_FIX_VOL_LOW_PANEL_ROUTE_2026_06_07: g_BTC_vol_low / g_ETH_vol_low /
        # g_J_btc_eth_vol_both_low declare a keyword-only `vol_hurst_panel`, but
        # were previously routed in the cross-asset / 2-asset confluence branches
        # below that pass ONLY `regime_panel` → missing-kwarg TypeError on every
        # eval → eval_failed → 0 fires (sol_15m v7/v8). They read vol, not
        # regime, so they belong here (removed from the regime branches below).
        if name.startswith((
            "g_vol_high",
            "g_BTC_vol_low", "g_ETH_vol_low", "g_J_btc_eth_vol_both_low",
        )):
        ...
        if name.startswith((
            ...
            "g_vwap_premium", "g_entry_vwap_not_coinflip",
        )):
            return {
                "slug": slot.slug,
                "book_mirror": self._book_mirror,
        ...
        # Slot-end OFI — directional Binance-spot taker-flow imbalance computed
        # at the gate from the 1s flow window (fire_us is only known there, like
        # g_disagr_hawkes). TV_FIX_SLOT_END_OFI_2026_06_07: was a return-False
        # stub fed only slug/slot_end_us; now fed vwap_store + asset so it can
        # actually evaluate. slug/slot_end_us kept for forensic context.
        ...
        # ORACLE-SETTLE (TV_AGENT_SPEC_SHADOW_ORACLE_SETTLE_2026_06_05) — the gate
        # needs the live chainlink price + the per-window STRIKE (chainlink at
        # slot_start, = the value the market settles against) + the winner-token
        # book. Both chainlink reads are sync module accessors on the TV-native
        # RTDS feed (run_rtds_feed runs in tv-engine; CLAUDE inv #13). The strike
        # is keyed by (coin, window_start_unix) = slot.ws_s.
        if name.startswith("g_oracle_decided"):
            from backend.app.services.polymarket_rtds import (
                get_boundary_snapshot as _get_strike,
                get_chainlink_price as _get_chainlink,
            )
            return {
                "book_mirror": self._book_mirror,
                "token_id_up": slot.token_id_up,
                "token_id_dn": slot.token_id_dn,
                "chainlink_price": _get_chainlink(sleeve.asset),
                "strike_price": _get_strike(sleeve.asset, slot.ws_s),
            }
    ...
    def _build_event(
        ...
        *,
        ...
        if event_type is None:
            ...
            # TV_AGENT_SPEC_SHADOW_DISAGR_HAWKES — tag every signal/eval/fire
            # row with a distinct event_type so it's filterable in trading.events
            # (resolution rows pass an explicit event_type and are unaffected).
            if sleeve.sleeve_id.startswith("shadow_disagr_hawkes"):
                event_type = "sleeve_disagr_hawkes"
            # TV_AGENT_SPEC_SHADOW_ORACLE_SETTLE — distinct event_type so the
            # structural settlement fires are filterable + never blur into the
            # lag-taker scalp WR/PnL (resolution rows still emit the canonical
            # poly_updown_resolution; this only tags signal/eval/placed rows).
            elif sleeve.sleeve_id.startswith("shadow_oracle_settle"):
                event_type = "sleeve_oracle_settle"
        ...
        # TV_FIX_WON_STAMP_2026_06_11 — scalp exits + early-cut rows have NO
        # Up/Down outcome, so ``fr.won`` is not the round-trip result and is
        # written null/False. Stamp ``won = (pnl > 0)`` at WRITE time for these
        # shapes so every consumer that reads the RAW row (ai_bot Telegram,
        # cross-box shadow sync, ad-hoc SQL) agrees with the dashboard's
        # ``normalize_resolution_payload`` (which already derives won=pnl>0 for
        # scalp/early-cut). Idempotent; only touches scalp/early-cut rows — the
        # hold-to-resolution path keeps ``won = outcome == direction`` untouched.
        if event_type == "sleeve_scalp_exit" or fr.exit_type in (
            "hedge_late_cut",
            "lag_reversal_cut",
        ):
            try:
                event["won"] = (float(pnl_usd) > 0) if pnl_usd is not None else False
            except (TypeError, ValueError):
                event["won"] = False
        if live:
            event["mode"] = "live"
        ...
        if event_type == "sleeve_fire_placed" and fr.fill_vwap is not None:
            ...
            # 2026-06-07 — durable entry_strike for the card (HOLD fix). 1m binance
            # close at window-start; box-independent (1m deque always on). slot.ws_s
            # is the authoritative window start (seconds). Never raises into the fire.
            try:
        ...
        # TV_AGENT_SPEC_SCALP_LAZER_DELTA_AB_2026_06_12 — δ-source telemetry on
        # every scalp/lazer row so the A/B is auditable: which feed drove δ, its
        # value, the live feed ages, and (lazer arms) the binance δ at the same
        # instant for per-fire disagreement. Stateless — reads current mirror +
        # store; ``delta_rtds_at_fire`` recomputes the binance return.
        _dsrc = getattr(sleeve, "delta_source", "rtds")
        if _dsrc == "lazer" or sleeve.exit_policy == "SCALP_EXIT":
            _da: dict[str, Any] = {
                "delta_source": _dsrc,
                "delta_value": fr.oracle_lag_bps,
                "lazer_age_ms": None,
                "rtds_age_ms": None,
                "delta_rtds_at_fire": None,
            }
            try:
                _mirror = get_lazer_instance()
                if _mirror is not None:
                    _da["lazer_age_ms"] = _mirror.age_ms(sleeve.asset)
            except Exception:  # noqa: BLE001
                pass
            try:
                _feed = get_feed_instance()
                _store = getattr(_feed, "vwap_store", None) if _feed else None
                _bars = getattr(_store, "bars", None) if _store else None
                _d = _bars.get(sleeve.asset) if _bars else None
                if _d:
                    _da["rtds_age_ms"] = (
                        (int(time.time() * 1e6) - _d[-1][0]) / 1000.0
                    )
            except Exception:  # noqa: BLE001
                pass
            if _dsrc == "lazer":
                try:
                    _rsnap = _binance_lag_snapshot(
                        sleeve.asset, slot.slot_start_us, fire_us,
                    )
                    _da["delta_rtds_at_fire"] = (
                        float(_rsnap.price_delta_bps)
                        if _rsnap is not None else None
                    )
                except Exception:  # noqa: BLE001
                    pass
            event["delta_ab"] = _da
        # TV_AGENT_SPEC_SHADOW_DISAGR_HAWKES — feature audit on every row so the
        # operator can verify fires match mp_skew<0 ∧ imb5_diff>0 ∧ hawkes<-0.2
        # (acceptance §4) + distinguish hawkes_warmup (hawkes None) from
        # no_signal. same_token_spread is the Down-token bid-ask the controller
        # gated on; cross_spread_old (in l25_book_snapshot) is logged-not-gated.
        if sleeve.sleeve_id.startswith("shadow_disagr_hawkes"):
            from backend.app.strategies.polymarket.sniper_v5_gates import (
                disagr_hawkes_features as _dh_feats,
            )
            _mp_skew, _imb5_diff = _dh_feats(
                self._book_mirror, slot.token_id_up, slot.token_id_dn,
            )
            _feed = get_feed_instance()
            _vw = getattr(_feed, "vwap_store", None) if _feed else None
            _same_spread: float | None = None
            _csnap = fr.l25_book_snapshot
            if _csnap is not None and _csnap.dn_ask0 is not None and _csnap.dn_bid0 is not None:
                _same_spread = float(_csnap.dn_ask0) - float(_csnap.dn_bid0)
            event["mp_skew"] = _mp_skew
            event["imb5_diff"] = _imb5_diff
            event["hawkes_lambda_imbalance"] = hawkes_lambda_imbalance_for_fire(
                _vw, sleeve.asset, fire_us,
            )
            event["same_token_spread"] = _same_spread
        return event

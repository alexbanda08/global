...
class SniperV5Sleeve:
    ...
    # V10 entry-price band (TV_AGENT_SPEC_EMA_DOWN_V10_2026_06_01.md) — a
    # POST-book-walk price gate: only fire when the bet-side entry vwap is in
    # ``[low, high)``. Skips the deep-lottery tail (< low: no edge, high
    # variance) AND the no-upside-favorite tail (>= high: win pays pennies,
    # loss costs full stake → -EV + tail risk). ``None`` = no band (default;
    # every existing sleeve unaffected). Checked by BOTH the Poly sniper-v5
    # controller (on the L25-walk fill_vwap) and the Kalshi controller (on the
    # no_ask DOWN entry price), after the book read, before placement.
    entry_band: tuple[float, float] | None = None
    # SCALP_EXIT (TV_AGENT_SPEC_SCALP_EXIT_SHADOW_2026_06_02) — intra-window
    # exit scalp on a lag-taker entry. SHADOW/paper only. When
    # exit_policy="SCALP_EXIT" the controller, after the paper buy fill, polls
    # the HELD side's best bid every ``scalp_poll_s`` and exits at the FIRST of:
    #   * TP   — best_bid >= ``scalp_tp_bid``                 (trigger "tp")
    #   * stop — best_bid <= fill_vwap - ``scalp_stop_delta`` (trigger "stop")
    #   * time — at fire_us + ``scalp_exit_offset_s``, sell at the bid ("time")
    # Empty book at the time exit → fall back to hold-to-resolution ("empty").
    # Default-valued → existing roster unaffected.
    scalp_exit_offset_s: int = 60
    scalp_tp_bid: float = 0.65
    scalp_stop_delta: float = 0.10
    scalp_poll_s: int = 5
    # SCALP_EXIT_CONFIG_BY_TF_2026_06_06 — TP and stop are SEPARATE switches:
    # scalp_tp_enabled (default False, taker-TP leaks edge → OFF) /
    # scalp_stop_enabled (default True, stop is protective → KEEP). The poll loop
    # still runs for the stop.
    scalp_tp_enabled: bool = False
    scalp_stop_enabled: bool = False  # 2026-06-11: stop REMOVED — corrected-harness rerun shows stop paired ON-OFF = -2.8/-3.2 SIG-NEG (old +0.88 was a harness artifact, BUGFIX_RERUN_RESULTS_2026_06_10). Exit = pure +60s time sell.
    # SCALP MAKER EXIT (TV_AGENT_SPEC_SCALP_MAKER_EXIT_2026_06_06) — Poly only.
    # "taker" | "maker_fixed" (rest a post-only SELL at scalp_maker_tp=0.60,
    # taker-+60 fallback) | "maker_peg". Poly 15m → maker_fixed@0.60; 5m + Kalshi
    # → taker. Maker fills earn the rebate share. Kalshi MUST stay taker.
    scalp_exit_mode: str = "taker"
    scalp_maker_tp: float = 0.60
    scalp_maker_repost_s: int = 5
    scalp_maker_rebate_share: float = 0.20
    # Min-book-events fill guard window (seconds). The controller requires
    # >= tv_poly_sniper_v5_fill_min_book_events_60s (default 25) BookMirror
    # events within this trailing window before a fire. Default 60s = existing
    # roster behavior; DISAGR-HAWKES sets 120s to match its validated gate
    # (TV_AGENT_SPEC_SHADOW_DISAGR_HAWKES_SOL5M: ">=25 in the 120s pre-fire window").
    book_event_window_s: float = 60.0
    # TV_AGENT_SPEC_SCALP_LAZER_DELTA_AB_2026_06_12 — δ price source for the
    # exit-scalp lag signal. "rtds" (default) = binance vwap_store return;
    # "lazer" = px@fire from the Pyth Lazer mirror (strike still binance@
    # slot_start). Stale lazer mirror -> SKIP (lazer.skipped_stale). SIGNAL only.
    delta_source: str = "rtds"
...
_SPREAD_SCALP_WIDE = {"BNB": Decimal("0.12"), "DOGE": Decimal("0.12")}  # gate-soften 2026-06-11
...
SNIPER_V5_SLEEVES: Final[tuple[SniperV5Sleeve, ...]] = (
    ...
    # V10 entry-band (TV_AGENT_SPEC_EMA_DOWN_V10_2026_06_01) applied IN-PLACE to
    # this LIVE sleeve per operator (no V10 clone): only fire when the DOWN
    # entry vwap ∈ [0.15, 0.93).
    SniperV5Sleeve(
    ...
    SniperV5Sleeve(
        ...
        gates=(
            ...
            # TV_AGENT_SPEC_CLOUDVWAP_V7_COINFLIP_FILTER_2026_06_09 — skip the
            # (0.49,0.51) book-walk-vwap coinflip dead-zone (no edge, -0.79/tr OOS).
            GateRef(g_entry_vwap_not_coinflip, (), "g_entry_vwap_not_coinflip"),
        ),
    ),
    ...
    # TV_AGENT_FIX_L1HRF_IMB5_GATE_2026_06_03 — gate-1 was wrongly wired to
    # g_grandparent_trend_with (a 5m→hi-tf trend slope) since first deploy
    # (91ce2f9), contradicting the "1hrf" name + validated config. Swap to the
    # 1h range-filter gate g_1h_rf_with (panel wired, controller:2211) and append
    # the entry-vwap cap g_entry_vwap_in_band ([0.20,0.80]) so it can't buy
    # over-priced favorites (the loss driver: 74-78% WR but −EV expensive entries).
    SniperV5Sleeve(
        ...
        spread_filter=_SPREAD_BTC,
        gates=(
            GateRef(g_1h_rf_with, (("asset", "BTC"),), "g_1h_rf_with(BTC)"),
            GateRef(g_imb5_strong_with, (), "g_imb5_strong_with"),
            GateRef(g_rf_with, (("asset", "BTC"),), "g_rf_with(BTC)"),
            GateRef(g_entry_vwap_in_band, (), "g_entry_vwap_in_band"),
        ),
    ),
    # V8_02 — BTC 5m BOTH offsets ALL — 1h RF + imb5 strong + ribbon (same gate-1
    # fix as rf_v8; gate-3 is the ribbon agreement instead of RF).
    # DEPRECATED-2026-06-08 SniperV5Sleeve(
        # DEPRECATED-2026-06-08 sleeve_id="poly_sniper_v5_btc_5m_l_1hrf_imb5_ribbon_v8",
        # DEPRECATED-2026-06-08 asset="BTC", tf="5m", direction="BOTH",
        # DEPRECATED-2026-06-08 offsets=(30, 60, 90, 120, 150, 180, 210, 240, 270),
        # DEPRECATED-2026-06-08 spread_filter=_SPREAD_BTC,
        # DEPRECATED-2026-06-08 gates=(
            # DEPRECATED-2026-06-08 GateRef(g_1h_rf_with, (("asset", "BTC"),), "g_1h_rf_with(BTC)"),
            # DEPRECATED-2026-06-08 GateRef(g_imb5_strong_with, (), "g_imb5_strong_with"),
            # DEPRECATED-2026-06-08 GateRef(g_ribbon_agrees, (("asset", "BTC"),), "g_ribbon_agrees(BTC)"),
            # DEPRECATED-2026-06-08 GateRef(g_entry_vwap_in_band, (), "g_entry_vwap_in_band"),
    ...
    # =================================================================
    # SCALP_EXIT shadow sleeves (TV_AGENT_SPEC_SCALP_EXIT_SHADOW_2026_06_02)
    # =================================================================
    # Intra-window EXIT-scalp on the CORRECTED lag-taker entry: buy the favored
    # token, then SELL it on the book at +60s (or TP@0.65 / stop@fill-0.10)
    # instead of holding to chainlink resolution. SHADOW/paper ONLY — graduation
    # gates (forward-OOS n>=200, live taker-sell fee, exit-fill realism) NOT yet
    # cleared; do NOT route real capital. Entry = delta_bps>=5 (g_oracle_lag_with
    # 5..12, the sharper subset), fire at +5s, $25, one-shot/slug, BOTH (the gate
    # fires the leading side). BTC+ETH x 5m+15m. The GATED v1 sleeves add the
    # robust cheap-entry cell entry_band=(0,0.55) (vwap<0.55, where the edge
    # survives the worst-case fee); the _control_v1 sleeves drop the band to
    # measure the gate's live lift. Resolution emits event_type='sleeve_scalp_exit'
    # (distinct → never double-counts in the main WR/PnL).
    *(
        SniperV5Sleeve(
            sleeve_id=f"shadow_scalp_exit_{_sym.lower()}_{_tf}{_ctl}",
            ...
            gates=(
                GateRef(
                    g_oracle_lag_with,
                    (("lo_bps", "5.0"), ("hi_bps", "12.0")),
                    "g_oracle_lag_with(5.0,12.0)",
                ),
            ),
        )
        for _sym in ("BTC", "ETH")
    ...
    # SCALP_EXIT δ≥3 VARIANT (TV_AGENT_SPEC_SCALP_DELTA3_VARIANT_2026_06_02)
    # -----------------------------------------------------------------
    # Higher-volume clone of the δ≥5 scalp sleeves above: identical mechanics
    # but a LOOSER entry gate (g_oracle_lag_with 3..12 instead of 5..12) → ~3.4×
    # fire rate, to reach the forward-OOS graduation gate (n>=200) faster. Stake
    # $5/fire (operator request). SHADOW/paper ONLY — same graduation gates as
    # the parent spec; do NOT route real capital. Backtest δ≥3 ALL n=398 WR 68.6%
    # worst-fee +$2.56/tr @$25 (CI excludes 0); ETH marginal (CI includes 0).
    *(
        SniperV5Sleeve(
            sleeve_id=f"shadow_scalp_exit_{_sym.lower()}_{_tf}_d3{_ctl}",
            asset=_sym, tf=_tf, direction="BOTH",
            offsets=(5,),
            spread_filter=_SPREAD_LAGV2,
            notional_usd_override=Decimal("5.0"),
            one_shot_per_slug=True,
            exit_policy="SCALP_EXIT",
            # SCALP_EXIT_CONFIG_BY_TF_2026_06_06: 15m → maker@0.60 + taker-+60
            # fallback; 5m → pure taker +60. Stop stays ON for both.
            scalp_exit_mode="taker",
            entry_band=None if _ctl == "_control_v1" else (0.0, 0.55),
            gates=(
                GateRef(
                    g_oracle_lag_with,
    ...
    # NO-TP SHADOW A/B TWINS (operator 2026-06-07) — paper mirrors of the LIVE
    # Ireland no-TP scalp sleeves (5m taker, 15m maker@0.60; stop ON, TP OFF) for
    # the live-vs-shadow A/B. Never live-promoted → always paper.
    SniperV5Sleeve(
        sleeve_id="shadow_scalp_exit_btc_5m_d3_notp_v1",
        asset="BTC", tf="5m", direction="BOTH",
        offsets=(5,),
        spread_filter=_SPREAD_LAGV2,
        notional_usd_override=Decimal("5.0"),
    ...
    SniperV5Sleeve(
        sleeve_id="shadow_scalp_exit_btc_15m_d3_notp_v1",
        asset="BTC", tf="15m", direction="BOTH",
        offsets=(5,),
        spread_filter=_SPREAD_LAGV2,
        notional_usd_override=Decimal("5.0"),
    ...
    # BTC 5m lag + momentum-alignment scalp
    # (TV_AGENT_SPEC_SCALP_MOMALIGN_BTC5M_2026_06_09). Refinement of the deployed
    # shadow_scalp_exit_btc_5m_d3_v1: SAME base lag scalp + a NEW momentum-
    # alignment gate, fired at offset 30/60s. Shadow $25 paper / live $1 probe.
    # BTC ONLY (ETH did not hold). Exit (validated): taker time exit +45s,
    # protective stop @ fill_vwap-0.10 ON, NO take-profit (TP leaks). ~91%
    # slug-disjoint from the deployed 5s scalp (additive, negligible overlap).
    SniperV5Sleeve(
        ...
        exit_policy="SCALP_EXIT",
        entry_band=(0.0, 0.55),
        scalp_exit_offset_s=45,    # taker time exit +45s (validated)
        scalp_stop_delta=0.10,     # protective stop @ fill_vwap-0.10 (KEEP — edge)
        scalp_stop_enabled=False,  # stop removed 2026-06-11 (see dataclass default)   # stop ON
        scalp_tp_enabled=False,    # NO take-profit (TP leaks $/tr 3.59->1.01)
        gates=(
            GateRef(
                g_oracle_lag_with,
                (("lo_bps", "3.0"), ("hi_bps", "12.0")),
            ...
            GateRef(
                g_lag_momentum_align,
                (("asset", "BTC"), ("mom_win_s", "30")),
                "g_lag_momentum_align(BTC,30)",
            ),
        ),
    ),
    # CONTROL — identical config but the momalign gate is OFF (oracle-lag only),
    # to measure the momalign gate's live lift (paired A/B, §6 graduation gate 2).
    # SHADOW only: NO live_notional_usd_override → never live-promoted.
    SniperV5Sleeve(
        ...
        exit_policy="SCALP_EXIT",
        entry_band=(0.0, 0.55),
        scalp_exit_offset_s=45,
        scalp_stop_delta=0.10,
        scalp_stop_enabled=False,  # stop removed 2026-06-11 (see dataclass default)
        scalp_tp_enabled=False,
        gates=(
            GateRef(
                g_oracle_lag_with,
                (("lo_bps", "3.0"), ("hi_bps", "12.0")),
    ...
    # =================================================================
    # LAZER δ A/B/L scalp sleeves (TV_AGENT_SPEC_SCALP_LAZER_DELTA_AB_2026_06_12)
    # =================================================================
    # Three arms × BTC/ETH/SOL × 5m/15m = 18 sleeves. IDENTICAL to the deployed
    # ``shadow_scalp_exit_*_d3_v1`` config (spread _SPREAD_LAGV2, $5, one-shot/
    # slug, SCALP_EXIT taker +60s time exit, entry_band (0,0.55), gate
    # g_oracle_lag_with(3,12)) EXCEPT the δ price source + the eval offset:
    #   * rtdsd (CONTROL) — δ from binance vwap_store @ fire (production path), +5s
    #   * lazerd (ARM L1) — δ px@fire from the Pyth Lazer mirror, +5s
    #                       (pure measurement-precision test vs CONTROL)
    #   * lazere (ARM L2) — δ px@fire from Lazer, +3s (latency-capture test)
    # Strike = binance@slot_start in ALL arms (only px@fire differs). SHADOW $0:
    # NO live_notional_usd_override → can never be live-promoted. On a stale Lazer
    # mirror the lazer arms SKIP (skip_reason="lazer.skipped_stale") — no rtds
    # fallback (would contaminate the A/B). SIGNAL only (Pyth != Chainlink).
    *(
        SniperV5Sleeve(
            sleeve_id=f"shadow_scalp_{_arm}_{_sym.lower()}_{_tf}",
            asset=_sym, tf=_tf, direction="BOTH",
            offsets=(_off,),
            spread_filter=_SPREAD_LAGV2,
            notional_usd_override=Decimal("5.0"),
            one_shot_per_slug=True,
            exit_policy="SCALP_EXIT",
            scalp_exit_mode="taker",   # PURE TAKER +60s time exit (TP/stop off)
            entry_band=(0.0, 0.55),
            delta_source=_dsrc,
            gates=(
                GateRef(
    ...
    # DISAGR-HAWKES (TV_AGENT_SPEC_SHADOW_DISAGR_HAWKES_SOL5M_2026_06_03) — the
    # one engine_v2-fill survivor (+$3.70/tr, t=2.84). SHADOW/paper only; DN
    # only; fires at slot_start+210s (late-window deep favorite, vwap≈0.87);
    # same-token (Down) bid-ask spread <=0.02 (NOT cross-token); one-shot/slug;
    # HOLD to resolution. Gate g_disagr_hawkes = mp_skew<0 ∧ imb5_diff>0 ∧
    # hawkes_lambda_imbalance<-0.2. Graduation bar = >=200 forward fires + CI>0.
    SniperV5Sleeve(
        ...
        notional_usd_override=Decimal("25.0"),
        one_shot_per_slug=True,
        book_event_window_s=120.0,  # validated gate: >=25 book events in 120s
        gates=(
            GateRef(g_dir_down, (), "g_dir_down"),
            GateRef(g_disagr_hawkes, (), "g_disagr_hawkes"),
        ),
    ),
    # =================================================================
    # TOD2 time-of-day gate (TV_AGENT_SPEC_SCALP_TOD_GATE_2026_06_05) — 4
    # =================================================================
    # Clone of the GATED scalp v1 sleeves + a UTC-hour exclude gate
    # g_hour_not_in(12,17) (TOD2 set, 94% coverage). Scalp edge ~2× in 22–02 UTC,
    # dead at h12/17; excluding lifts $/tr (+2.95→+3.14) ~no volume loss (3/3
    # walk-forward). CONTROL = the matching un-houred shadow_scalp_exit_*_v1.
    # SHADOW/paper only. btc_5m δ≥5 ($25) + the $5 δ≥3 cells. Exit +60s.
    *(
        SniperV5Sleeve(
            ...
            offsets=(5,),
            spread_filter=_SPREAD_LAGV2,
            notional_usd_override=Decimal("5.0") if _d3 else Decimal("25.0"),
            one_shot_per_slug=True,
            exit_policy="SCALP_EXIT",
            entry_band=(0.0, 0.55),
            gates=(
        ...
        for (_sym, _tf, _d3) in (
            ("BTC", "5m", ""), ("BTC", "5m", "_d3"),
            ("BTC", "15m", "_d3"), ("ETH", "5m", "_d3"),
        )
    ),
    # =================================================================
    # ALL-COINS exit-scalp (TV_AGENT_SPEC_SCALP_ALLCOINS_2026_06_05) — 24
    # =================================================================
    # Extend the BTC/ETH exit-scalp to SOL, DOGE, BNB. Disjoint-window OOS PASS:
    # SOL +$2.16, DOGE +$1.40 (gated CI>0); BNB +$1.39 directional (accrue power).
    # Identical mechanics: lag-taker +5s, SELL +60s (SCALP_EXIT), one-shot/slug,
    # BOTH, gated entry_band=(0,0.55) + control. $5 ONLY (thin-book friendly).
    # NO TOD gate (22–02 boost didn't replicate on DOGE/BNB). SHADOW/paper only.
    # Needs DOGE/BNB binance 1s spot (BINANCE_TV_MAP) + DOGE/BNB in the sniper_v5
    # slot_observer (main.py) + book-mirror auto-subs from discovered tokens.
    *(
        SniperV5Sleeve(
            sleeve_id=f"shadow_scalp_exit_{_sym.lower()}_{_tf}{_dl}{_ctl}",
            asset=_sym, tf=_tf, direction="BOTH",
            offsets=(5,),
            # 2026-06-11 gate-soften (GATE_SOFTEN_BNB_XRP_DOGE_2026_06_11): the lag
            # edge on thin alts lives in the WIDE-spread open book — incremental
            # (0.05,0.12] fires are CI>0 on BNB (+4.93/tr) and DOGE (+2.74/tr).
            # XRP stays 0.05 (incremental insignificant). Delta floor unchanged.
            spread_filter=_SPREAD_SCALP_WIDE.get(_sym, _SPREAD_LAGV2),
            notional_usd_override=Decimal("5.0"),  # thin-book friendly
            one_shot_per_slug=True,
            exit_policy="SCALP_EXIT",
            entry_band=None if _ctl == "_control_v1" else (0.0, 0.55),
            gates=(
                GateRef(
                    g_oracle_lag_with,
                    (("lo_bps", "3.0" if _dl == "_d3" else "5.0"),
                     ("hi_bps", "12.0")),
                    f"g_oracle_lag_with({'3.0' if _dl == '_d3' else '5.0'},12.0)",
                ),
            ),
        )
        for _sym in ("SOL", "DOGE", "BNB", "XRP")  # XRP added 2026-06-05 (OOS +$2.20)
        for _tf in ("5m", "15m")
        for _dl in ("_d3", "")           # "" = δ≥5
        for _ctl in ("_v1", "_control_v1")
    ),
    # =================================================================
    # ORACLE-DETERMINISM SETTLEMENT (TV_AGENT_SPEC_SHADOW_ORACLE_SETTLE_2026_06_05) — 6
    # =================================================================
    # STRUCTURAL/settlement edge (NOT prediction). ~18% of slugs decided by
    # Chainlink ≥99.6% at T-60s; the oracle-winner poly price LAGS the oracle
    # (+1.35%/share EV), concentrated in CHEAP-but-decided slugs. UNDERPOWERED
    # offline — accrue forward fills + a live RTDS-fidelity probe. Fire at T-60s
    # (offset window_s-60: 5m→240, 15m→840); BOTH (gate sets the winner side);
    # HOLD to resolution (default exit_policy). $5, one-shot/slug, paper-only.
    # SOL included (cheap-decided strongest on ETH/SOL thin books). Fire rows tag
    # event_type 'sleeve_oracle_settle'. Needs live RTDS chainlink + per-window
    # strike (get_boundary_snapshot), both already in tv-engine.
    *(
        SniperV5Sleeve(
            sleeve_id=f"shadow_oracle_settle_{_sym.lower()}_{_tf}",
            asset=_sym, tf=_tf, direction="BOTH",
            offsets=(_off,),                       # window_s - 60 (T-60s)
        ...
        for (_sym, _tf, _off) in (
            ("BTC", "5m", 240), ("ETH", "5m", 240), ("SOL", "5m", 240),
            ("BTC", "15m", 840), ("ETH", "15m", 840), ("SOL", "15m", 840),
        )
    ),
    # =================================================================
    # ETH MERGE — cloud_vwap_hurstmp ∧ ema50_hurst_grandparent (CONVICTION AND)
    # (TV_AGENT_SPEC_ETH_MERGE_HURST_2026_06_08) — SHADOW/paper $5.
    # =================================================================
    # Candidate = cloud_vwap_hurstmp_v7 (3 gates copied below) + ONE confirm gate
    # requiring sibling ema50_hurst_grandparent_v8 to ALSO pass its full gate-stack
    # on the SAME slug+fire_us AND the SAME direction. Sibling gates are NOT forked
    # — evaluated live by id. HOLD-to-resolution (no scalp exit). Control = cloud
    # SOLO (_v7). Evidence: NEW_EDGE_RESEARCH_2026_06_08 Thread A ($/tr +0.62 vs +0.33).
    SniperV5Sleeve(
...
SNIPER_V5_SLEEVE_BY_ID: Final[dict[str, SniperV5Sleeve]] = {
    s.sleeve_id: s for s in SNIPER_V5_SLEEVES
}

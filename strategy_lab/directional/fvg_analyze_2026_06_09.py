"""Analyze the FVG grid: WHERE does the edge live (offset), does a gap threshold + cheap-gate beat noise,
   and is there a MID-WINDOW pocket? Reads scalp_fvg_grid_2026_06_09{TAG}.parquet."""
import sys, os, glob
from pathlib import Path
import numpy as np, pandas as pd
np.random.seed(0)
ROOT = Path(r"C:\Users\alexandre bandarra\Desktop\global"); RES = ROOT / "strategy_lab/directional/_results"
G = pd.concat([pd.read_parquet(f) for f in glob.glob(str(RES / "scalp_fvg_grid_2026_06_09*.parquet"))], ignore_index=True)

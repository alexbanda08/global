"""
Fetch SUIUSDT and TONUSDT from Binance Vision.
Produces: data/binance/parquet/{SYM}/{TF}/year=YYYY/part.parquet
Same layout as existing coins (fetch_binance_multi.py).
"""
from __future__ import annotations
import io, logging, sys, zipfile
...
SYMBOLS_STARTS = {
    "SUIUSDT": date(2023, 5, 1),
    "TONUSDT": date(2022, 10, 1),
}
...
log = logging.getLogger("fetch-sui-ton")
...
def main():
    tasks = []
    for sym, start in SYMBOLS_STARTS.items():
        for tf in INTERVALS:
            for y, m in month_iter(start, END):
                tasks.append((sym, tf, y, m))

    log.info(f"queued {len(tasks)} month-downloads for {list(SYMBOLS_STARTS)}")
    ...
    # Rebuild parquet for any year with new zips or missing parquet.
    rebuild = set()
    for sym in SYMBOLS_STARTS:
        for tf in INTERVALS:

...
Same algorithm as migration_2026_05_26/consolidate_l25_to_canonical.py — just
adds the new refresh_2026_05_27 path to the SOURCES list.
"""
...
SOURCES = [
    ...
    ("refresh_2026_05_27/cache",      "_orderbook_L25_topoff.parquet"),
]
...
def consolidate_asset(asset: str):
    ...
    for subdir, src_path in sources_present:
        ...
        for batch in pf.iter_batches(batch_size=BATCH_SIZE, columns=TARGET_COLS):
            ...
            outcomes = batch.column("outcome").to_pylist()
    ...
    print(f"  [{asset}] CONSOLIDATED -> {out_path.name}  rows={md_rows:,}  rg={pf_out.metadata.num_row_groups}  size={sz:.0f}MB")

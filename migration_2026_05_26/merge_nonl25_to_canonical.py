"""
Merge non-L25 deltas from refresh_2026_05_26/cache/ into canonical/.
Rebuilds resolutions_from_rtds at the end.
(L25 was already consolidated separately via consolidate_l25_to_canonical.py.)
"""
...
if 'at' in new.columns:
    at = pd.to_datetime(new['at'], utc=True, format='mixed', errors='coerce')
    log(f"  span: {at.min()}  ->  {at.max()}")

...
class SniperV5Sleeve:
    ...
    # SCALP_EXIT (TV_AGENT_SPEC_SCALP_EXIT_SHADOW / SCALP_LIVE) — intra-window
    # exit scalp on a lag-taker entry. When exit_policy="SCALP_EXIT" the loop,
    # after the buy fill, polls the held side's best bid every ``scalp_poll_s``
    # and exits at the FIRST of: TP (best_bid >= ``scalp_tp_bid``), stop
    # (best_bid <= fill_vwap - ``scalp_stop_delta``), or time (at fire_us +
    # ``scalp_exit_offset_s``). Default-valued → existing roster unaffected.
    ...
    #   * scalp_tp_enabled (default False): taker-TP@scalp_tp_bid leaks edge → OFF.
    #   * scalp_stop_enabled (default True): stop@(fill_vwap-scalp_stop_delta) is
    #     protective +EV → KEEP ON. The poll loop still runs for the stop.
    ...
    # "taker" (current) | "maker_fixed" (rest a post-only SELL at scalp_maker_tp,
    # 0.60 = validated 15m target, taker-+60 fallback) | "maker_peg" (rest at the
    # best ask). Per SCALP_EXIT_CONFIG_BY_TF: Poly 15m → maker_fixed@0.60; Poly 5m
    # + Kalshi → taker. Maker fills earn the rebate share. Kalshi MUST stay taker.
    ...
    # Per-sleeve LIVE stake override (TV_AGENT_SPEC_SCALP_LIVE_2026_06_05). When
    # set AND this sleeve is live-promoted (live allowlist + live_enabled), the
    # controller sizes the REAL entry off THIS value instead of the shared
    # ``tv_poly_sniper_v5_live_notional_usd``. Still hard-clamped to
    # ``SNIPER_V5_LIVE_MAX_NOTIONAL`` ($2). None → global live stake (existing
    # live sleeves unaffected). Paper fires ignore this field.
    live_notional_usd_override: Decimal | None = None
...
SNIPER_V5_SLEEVES: Final[tuple[SniperV5Sleeve, ...]] = (
    ...
    # V8_02 — BTC 5m BOTH offsets ALL — 1h RF + imb5 strong + ribbon.
    # PORT 2026-06-07 (operator): gate-1 g_grandparent_trend_with -> g_1h_rf_with
    # + g_entry_vwap_in_band cap, matching the validated VPS3 shadow config.
    # LIVE-promoted at $1 via the allowlist (one-shot-per-slug, hold-to-resolution).
    # DEPRECATED-2026-06-08 SniperV5Sleeve(
        ...
        # DEPRECATED-2026-06-08 live_notional_usd_override=Decimal("1.0"),
    ...
    # LIVE scalp-exit (TV_AGENT_SPEC_SCALP_LIVE_2026_06_05) — promoted to LIVE on
    # Ireland ($1 probe). Lag-taker entry +5s, SELL on the book at +60s/TP/stop;
    # δ≥3 gate (g_oracle_lag_with 3..12), cheap-entry band (0,0.55). The live
    # entry + live SELL leg size off live_notional_usd_override ($1, clamped $2).
    # Goes live only when allowlisted in TV_POLY_SNIPER_V5_LIVE_ALLOWLIST.
    SniperV5Sleeve(
        sleeve_id="shadow_scalp_exit_btc_5m_d3_v1",
        asset="BTC", tf="5m", direction="BOTH",
        offsets=(5,),
        spread_filter=_SPREAD_LAGV2,
        notional_usd_override=Decimal("5.0"),
    ...
    # LIVE scalp MAKER exit (SCALP_EXIT_CONFIG_BY_TF + TV_AGENT_SPEC_SCALP_MAKER_EXIT,
    # 2026-06-07) — Poly 15m promoted to LIVE on Ireland ($1 probe). Same δ≥3 lag
    # entry as the 5m, but the take-profit is a MAKER@0.60 offer (post-only SELL,
    # fee 0 + rebate) with a taker-+60 fallback; the protective stop@-0.10 stays
    # on. Live only when allowlisted in TV_POLY_SNIPER_V5_LIVE_ALLOWLIST.
    SniperV5Sleeve(
        sleeve_id="shadow_scalp_exit_btc_15m_d3_v1",
        asset="BTC", tf="15m", direction="BOTH",
        offsets=(5,),
        spread_filter=_SPREAD_LAGV2,
        notional_usd_override=Decimal("5.0"),

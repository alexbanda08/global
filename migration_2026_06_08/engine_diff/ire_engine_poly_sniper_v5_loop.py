...
async def _scalp_exit_then_resolve(
    ...
    exited = False
    # SCALP_EXIT_CONFIG_BY_TF_2026_06_06: poll until the +60 deadline for the
    # protective STOP (the taker-TP is gated off in the controller, but the stop
    # stays on). MAKER mode (Poly 15m, TV_AGENT_SPEC_SCALP_MAKER_EXIT) additionally
    # rests a maker SELL on entry and checks its fill each poll; the controller
    # method routes poll→stop/maker and deadline→cancel+taker-fallback.

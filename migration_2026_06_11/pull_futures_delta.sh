...
sudo -u postgres psql -d storedata -v ON_ERROR_STOP=1 -c "\copy (SELECT * FROM cex_futures_klines_v2 WHERE time_period_start_us >= ${T_START_US}) TO '${OUT}' CSV HEADER"
echo "  rows: $(wc -l < $OUT) ($(($(date +%s)-T0))s)"; gzip -f $OUT

OUT=$DIR/cex_futures_ticker.csv; echo "[cex_futures_ticker_v2]"; T0=$(date +%s)
sudo -u postgres psql -d storedata -v ON_ERROR_STOP=1 -c "\copy (SELECT * FROM cex_futures_ticker_v2 WHERE time_exchange_us >= ${T_START_US}) TO '${OUT}' CSV HEADER"
echo "  rows: $(wc -l < $OUT) ($(($(date +%s)-T0))s)"; gzip -f $OUT

OUT=$DIR/cex_futures_trades.csv; echo "[cex_futures_trades_v2]"; T0=$(date +%s)
sudo -u postgres psql -d storedata -v ON_ERROR_STOP=1 -c "\copy (SELECT * FROM cex_futures_trades_v2 WHERE time_exchange_us >= ${T_START_US}) TO '${OUT}' CSV HEADER"
echo "  rows: $(wc -l < $OUT) ($(($(date +%s)-T0))s)"; gzip -f $OUT

for EX in gate okx; do
  OUT=$DIR/${EX}_liquidations.csv; echo "[${EX}_liquidations_v2]"; T0=$(date +%s)
  sudo -u postgres psql -d storedata -v ON_ERROR_STOP=1 -c "\copy (SELECT * FROM ${EX}_liquidations_v2 WHERE time_exchange_us >= ${T_START_US}) TO '${OUT}' CSV HEADER"
  echo "  rows: $(wc -l < $OUT) ($(($(date +%s)-T0))s)"; gzip -f $OUT
done

...
# Phase 33 — Deprecation set. Read TV_POLY_DEPRECATED_SLEEVES (CSV) at
# module import. The on_bar_close wrapper constructs the canonical
# sleeve_id for the current (sym, tf, strategy_mode[, hedge_policy])
# and early-returns if it's in this set — no compute, no audit row.
# Module-level read (not from bots.py) to avoid circular import. Same
# env var, same lifetime contract as bots._DEPRECATED_POLY_UPDOWN_SLEEVE_IDS.
_DEPRECATED_SLEEVE_IDS_AT_DISPATCH: frozenset[str] = frozenset(
    s.strip()
...
#: Legacy flat fee baseline — kept for downward compat (only used when
#: shares + price are both missing from the fill response, which V2 never
#: should produce). Real fee comes from compute_taker_fee_usd below.
_TV_POLY_FEE_USD_PER_FILL = _f_env("TV_POLY_FEE_USD_PER_FILL", 0.0)
...
#: Market category for Polymarket UpDown markets (crypto-binary on
#: BTC/ETH/SOL). Drives the feeRate selection in compute_taker_fee_usd.
#: Per https://docs.polymarket.com/trading/fees: Crypto → 0.07 / 20% rebate.
_TV_POLY_MARKET_CATEGORY = os.environ.get(
    "TV_POLY_MARKET_CATEGORY", "crypto"
...
def _spread_pct(book: Any) -> float | None:
    """Compute ``(best_ask − best_bid) / mid`` from a CLOB book.

    Accepts EITHER shape (Phase 26.3 + live-mirror compat):
      * dict — ``{bids: [{price, size}], asks: [{price, size}]}``
               (paper-mode + BookMirror dict shape).
      * Pydantic ``OrderbookSnapshot`` — ``bids/asks: list[tuple[Decimal, Decimal]]``
               (LIVE PolymarketClient.get_orderbook_snapshot return type).

    The mismatch between paper (dict) and live (Pydantic) was latent until
    eth_5m_v3_2 became the first v3-family sleeve promoted to live on Ireland
    (2026-05-27) — the v3 spread filter is the only path that called this
    helper with the live return type. Both shapes now normalize to the same
    best-bid/best-ask Decimal extraction.
    ...
    if book is None:
        return None
    # Branch 1: Pydantic OrderbookSnapshot (live path). bids/asks are
    # list[(Decimal, Decimal)] tuples — best at index 0.
    if hasattr(book, "bids") and hasattr(book, "asks") and not isinstance(book, dict):
        bids_seq = book.bids
        asks_seq = book.asks
        if not bids_seq or not asks_seq:
            return None
        try:
            bid_raw = bids_seq[0][0]
            ask_raw = asks_seq[0][0]
            bid = Decimal(str(bid_raw))
            ask = Decimal(str(ask_raw))
        except (TypeError, IndexError, KeyError, ValueError, InvalidOperation):
            return None
    # Branch 2: dict shape (paper + BookMirror).
...
class PolymarketUpdownController:
    ...
    def __init__(
        ...
        *,
        ...
        # Phase 34 — shadow gated sleeves (TV_AGENT_SHADOW_DEPLOY_GATED_SLEEVES_2026_05_22.md).
        # When ``gate_stack`` is non-empty, after the underlying strategy's
        # ``signal()`` returns UP/DOWN AND F7 (if active) passes, the listed
        # gates run in order. Any gate skip → audit row with reason=
        # ``gate_{name}_skip`` and the dispatch returns. ``gate_cell_strategy``
        # is the key used to look up ``HOD_TOP8_BY_CELL`` (e.g., "sniper",
        # "momo", "momo_v2") — distinct from the controller's strategy_mode
        # because (e.g.) a sniper-base shadow may run as strategy_mode=sniper
        # but the HoD table is keyed by the same name. ``audit_sleeve_id``
        # overrides the standard sleeve_id construction in ``_audit`` so
        # the shadow sleeve writes to its own trading.events row stream
        # (e.g., ``poly_updown_sol_5m_sniper_hod_HOLD``) instead of the
        # base controller's id.
        gate_stack: tuple[str, ...] = (),
        gate_cell_strategy: str | None = None,
        audit_sleeve_id: str | None = None,
        # Phase 35 — VWAP continuation config. Required when
        # ``strategy_mode == "vwap_continuation"``; ignored otherwise.
        # The scheduler reads ``vwap_asset`` + ``vwap_tf`` + ``vwap_offset_s``
        # off the controller to group dispatches by (tf, offset) cell.
        vwap_offset_s: int | None = None,
        vwap_thr_min_bps: float | None = None,
        vwap_thr_max_bps: float | None = None,
        vwap_require_m1v: bool = False,
        vwap_require_f7: bool = False,
        vwap_require_cross_full: bool = False,
        vwap_require_cross_partial: bool = False,
        vwap_asset: str | None = None,
        vwap_tf: str | None = None,
        # Phase 36b — fade companion + overlay filter config. Used by the
        # 6 fade sleeves (Sleeves #2-7) and 6 overlay sleeves from
        # SHADOW_DEPLOY_SPEC_9_NEW_SLEEVES_2026_05_24.md.
        # ``fade_cell_key`` selects the HoD top-8 set for the cell
        #   (e.g., ``"sniper_btc_5m"``). Required for fade_* modes.
        # ``overlay_gate_kind`` selects the extra gate predicate.
        #   One of: ``"m5v_pass"``, ``"fairedge500"``,
        #   ``"fairedge500_cvd30"``, ``"cvd_macd"``. Required for overlay_* modes.
        fade_cell_key: str | None = None,
        overlay_gate_kind: str | None = None,
    ) -> None:
        ...
        # Phase 34 — shadow gated sleeve config. Validated lazily; an empty
        # gate_stack means this is a baseline controller (no behavior change).
        # ``_audit_sleeve_id_override`` is read at _audit time and (when set)
        # bypasses the base / momo / momo_v2 sleeve_id construction.
        _VALID_GATES = frozenset({"hod", "mtf2", "m5va", "m1va"})
        for _g in gate_stack:
            if _g not in _VALID_GATES:
                raise ValueError(
                    f"unknown gate {_g!r} in gate_stack; "
                    f"expected subset of {sorted(_VALID_GATES)}"
                )
        self._gate_stack: tuple[str, ...] = tuple(gate_stack)
        self._gate_cell_strategy: str | None = gate_cell_strategy
        self._audit_sleeve_id_override: str | None = audit_sleeve_id
        if self._gate_stack and self._gate_cell_strategy is None:
            raise ValueError(
                "gate_stack requires gate_cell_strategy "
                "(e.g., 'sniper' / 'momo' / 'momo_v2')"
            )
        # Per-(sym, ws_s) Markov regime cache. Spec §3.3: avoid recompute
        # across HOLD/HEDGE/SELL clones of the same boundary. Cleared lazily
        # when the cache key drifts (sym changes or ws_s advances). Small
        # dict — at most one entry per active boundary per symbol.
        self._markov_regime_cache: dict[tuple[str, int], int] = {}
        # Set by on_bar_close gate stack — read by _audit to attach the
        # full gate_decisions record to the order_placed audit payload.
        # Always reset to None at the END of _audit so the next dispatch
        # starts clean (concurrent dispatches share the controller instance
        # but never overlap because on_bar_close is serial per controller).
        self._gate_decisions_active: dict[str, dict[str, Any]] | None = None
        ...
        if strategy_mode == "momo":
        ...
        elif strategy_mode == "vwap_continuation":
            # Phase 35 — VWAP continuation strategy. Per-instance config
            # (offset_s, thr_min/max_bps, require_*) is locked at controller
            # spawn time. The strategy class is timeframe-agnostic; one
            # instance per (sym, tf) shares the same params. Slot allowlist
            # restricts dispatch to the controller's target cell only.
            if vwap_offset_s is None or vwap_thr_min_bps is None or vwap_thr_max_bps is None:
                raise ValueError(
                    "strategy_mode='vwap_continuation' requires "
                    "vwap_offset_s, vwap_thr_min_bps, vwap_thr_max_bps"
                )
            from backend.app.strategies.polymarket.vwap_continuation import (
                VwapContinuationStrategy,
            )
            _vwap_strategy = VwapContinuationStrategy(
                offset_s=vwap_offset_s,
                thr_min_bps=vwap_thr_min_bps,
                thr_max_bps=vwap_thr_max_bps,
                require_m1v=vwap_require_m1v,
                require_f7=vwap_require_f7,
                require_cross_full=vwap_require_cross_full,
                require_cross_partial=vwap_require_cross_partial,
            )
            for sym in SUPPORTED_SYMBOLS:
                self._strategies[(sym, "5m")] = _vwap_strategy
                self._strategies[(sym, "15m")] = _vwap_strategy
            # Stash config on the controller so the scheduler can group by
            # (tf, offset_s) cell and audit can read fire_offset_s.
            self.vwap_offset_s: int | None = vwap_offset_s
            self.vwap_thr_min_bps: float | None = vwap_thr_min_bps
            self.vwap_thr_max_bps: float | None = vwap_thr_max_bps
            self.vwap_asset: str | None = (
                vwap_asset.upper() if vwap_asset is not None else None
            )
            self.vwap_tf: str | None = (
                vwap_tf.lower() if vwap_tf is not None else None
            )
        elif strategy_mode == "vwap_kelly_ensemble":
            # Phase 36 Sleeve #1 — S4 ∪ S8 rule + Kelly tier. Same instance
            # for all (sym, tf) since strategy is timeframe-agnostic and
            # stateless. Phase gate inside signal() rejects non-t_plus_120+.
            from backend.app.strategies.polymarket.shadow9 import (
                VwapKellyEnsembleStrategy,
            )
            _kelly_strategy = VwapKellyEnsembleStrategy()
            for sym in SUPPORTED_SYMBOLS:
                self._strategies[(sym, "5m")] = _kelly_strategy
                self._strategies[(sym, "15m")] = _kelly_strategy
        elif strategy_mode == "prewindow_s3":
            # Phase 36 Sleeve #8 — S3 rule at slot_start - 60s, 5m only.
            from backend.app.strategies.polymarket.shadow9 import (
                PrewindowS3Strategy,
            )
            _s3_strategy = PrewindowS3Strategy()
            for sym in SUPPORTED_SYMBOLS:
                self._strategies[(sym, "5m")] = _s3_strategy
                self._strategies[(sym, "15m")] = _s3_strategy
        elif strategy_mode == "prewindow_s4_15m":
            # Phase 36 Sleeve #9 — S4 rule at slot_start - 120s, 15m only.
            from backend.app.strategies.polymarket.shadow9 import (
                PrewindowS4Strategy,
            )
            _s4_strategy = PrewindowS4Strategy()
            for sym in SUPPORTED_SYMBOLS:
                self._strategies[(sym, "5m")] = _s4_strategy
                self._strategies[(sym, "15m")] = _s4_strategy
        elif strategy_mode in ("fade_sniper", "fade_momo_v2"):
            # Phase 36b Sleeves #2-7 — wrap a production strategy + apply
            # HoD + M5V gates. Inverts direction when those gates would
            # have filtered the production fire.
            if fade_cell_key is None:
                raise ValueError(
                    f"strategy_mode={strategy_mode!r} requires fade_cell_key"
                )
            from backend.app.strategies.polymarket.shadow9 import (
                FadeCompanionStrategy,
            )
            if strategy_mode == "fade_sniper":
                # Base = Updown5mStrategy(mode="sniper") for 5m,
                #        Updown15mStrategy(mode="sniper") for 15m.
                for sym in SUPPORTED_SYMBOLS:
                    self._strategies[(sym, "5m")] = FadeCompanionStrategy(
                        base_strategy=Updown5mStrategy(mode="sniper"),
                        cell_key=fade_cell_key,
                    )
                    self._strategies[(sym, "15m")] = FadeCompanionStrategy(
                        base_strategy=Updown15mStrategy(mode="sniper"),
                        cell_key=fade_cell_key,
                    )
            else:  # fade_momo_v2
                from backend.app.strategies.polymarket import MomoV2Strategy
            ...
            self.fade_cell_key: str | None = fade_cell_key
        elif strategy_mode in ("overlay_sniper", "overlay_momo", "overlay_momo_v2"):
            # Phase 36b — wrap a production strategy + apply an EXTRA
            # gate predicate. Pass-through (NOT invert) when gate ok.
            if overlay_gate_kind is None:
                raise ValueError(
                    f"strategy_mode={strategy_mode!r} requires overlay_gate_kind"
                )
            from backend.app.strategies.polymarket.shadow9 import (
                OverlayFilterStrategy,
            )
            if strategy_mode == "overlay_sniper":
                for sym in SUPPORTED_SYMBOLS:
                    self._strategies[(sym, "5m")] = OverlayFilterStrategy(
                        base_strategy=Updown5mStrategy(mode="sniper"),
                        gate_kind=overlay_gate_kind,
                    )
                    self._strategies[(sym, "15m")] = OverlayFilterStrategy(
                        base_strategy=Updown15mStrategy(mode="sniper"),
                        gate_kind=overlay_gate_kind,
                    )
            elif strategy_mode == "overlay_momo":
                from backend.app.strategies.polymarket import MomoStrategy
                for sym in SUPPORTED_SYMBOLS:
                    self._strategies[(sym, "5m")] = OverlayFilterStrategy(
                        base_strategy=MomoStrategy(),
                        gate_kind=overlay_gate_kind,
                    )
                    self._strategies[(sym, "15m")] = OverlayFilterStrategy(
                        base_strategy=MomoStrategy(),
            ...
            self.overlay_gate_kind: str | None = overlay_gate_kind
        else:
            for sym in SUPPORTED_SYMBOLS:
        ...
        # Phase 35 — default VWAP attrs so the scheduler's getattr() doesn't
        # raise on non-vwap controllers. Only the vwap_continuation branch
        # above sets these to non-None.
        if not hasattr(self, "vwap_offset_s"):
            self.vwap_offset_s = None
            self.vwap_thr_min_bps = None
            self.vwap_thr_max_bps = None
            self.vwap_asset = None
            self.vwap_tf = None
    ...
    def _inject_fade_overlay_ref(self, aux: dict[str, Any]) -> dict[str, Any]:
        """For fade_* / overlay_* modes, wire ``_controller_ref`` into aux
        so :class:`FadeCompanionStrategy` / :class:`OverlayFilterStrategy`
        can read Phase 34 + Phase 36 features off the BarContext.

        Idempotent + safe for all strategy_modes.
        """
        if self.strategy_mode in (
            "fade_sniper", "fade_momo_v2",
            "overlay_sniper", "overlay_momo", "overlay_momo_v2",
        ):
            aux["_controller_ref"] = self
        return aux
    ...
    async def _build_signal_aux(self, symbol: str, tf: str, window_start_us: int) -> dict[str, Any]:
        ...
        # Phase 36b — fade/overlay modes alias to their base for aux shape.
        # The base build still runs; the wrapper strategy adds the
        # gate/invert logic on top. Enrichment for fade/overlay-specific
        # fields happens at the end of this function via _enrich_for_fade_overlay.
        _mode_for_aux = self.strategy_mode
        if _mode_for_aux in ("fade_sniper", "overlay_sniper"):
            _mode_for_aux = "sniper"
        elif _mode_for_aux in ("fade_momo_v2", "overlay_momo_v2"):
            _mode_for_aux = "momo_v2"
        elif _mode_for_aux == "overlay_momo":
            _mode_for_aux = "momo"

        if _mode_for_aux == "momo":
            ctx_phase = (
            ...
            if (
                ...
                return self._inject_fade_overlay_ref({
        ...
        # Phase 36 — Kelly ensemble + pre-window strategies consume a flat
        # aux dict built from the BarContext's Phase 36 fields. Phase gate
        # is enforced inside the strategy class against ``bar_ctx_phase``.
        if self.strategy_mode in (
            "vwap_kelly_ensemble",
            "prewindow_s3",
            "prewindow_s4_15m",
            ...
            # Mismatched cell (multi-cell controller — Sleeve #1 fires
            # BTC+ETH+SOL) STILL needs the bar_ctx fields when symbol
            # matches. Cross-asset audit happens at the strategy layer.
            if (
                _bar_ctx_pre is None
                or getattr(_bar_ctx_pre, "symbol", None) != sym_upper
                or getattr(_bar_ctx_pre, "tf", None) != tf
            ):
                return {"bar_ctx_phase": ctx_phase}
            # Strategy reads fair_edge_bp_up vs fair_edge_bp_down. The
            # BarContext stores the UP leg in ``fair_edge_bp``; we also
            # surface the DOWN leg here by recomputing from fair_up +
            # entry_vwap_no (cheap; one multiply).
            fe_up = getattr(_bar_ctx_pre, "fair_edge_bp", None)
            fu = getattr(_bar_ctx_pre, "fair_up_value", None)
            evn = getattr(_bar_ctx_pre, "entry_vwap_no", None)
            fe_dn: float | None = None
            ...
            return {
                "bar_ctx_phase": ctx_phase,
                "vwap_dev_bps": getattr(_bar_ctx_pre, "vwap_dev_bps", None),
                "fair_edge_bp": fe_up,
                "fair_edge_bp_down": fe_dn,
                "fair_up": fu,
                "cvd_30s": getattr(_bar_ctx_pre, "cvd_30s", None),
                "cvd_60s": getattr(_bar_ctx_pre, "cvd_60s", None),
                "macd_hist_value": getattr(_bar_ctx_pre, "macd_hist_value", None),
                "rvol_30_300": getattr(_bar_ctx_pre, "rvol_30_300", None),
                "sigma": getattr(_bar_ctx_pre, "sigma_per_sqrt_sec_15m", None),
                "s_now": getattr(_bar_ctx_pre, "s_now", None),
                "strike": getattr(_bar_ctx_pre, "strike", None),
                "tau_s": getattr(_bar_ctx_pre, "tau_s", None),
                "entry_vwap_yes": getattr(_bar_ctx_pre, "entry_vwap_yes", None),
                "entry_vwap_no": evn,
                "_controller_ref": self,
            }
        ...
        return self._inject_fade_overlay_ref(aux)
    ...
    async def on_bar_close(
        ...
        *,
        ...
        if (
            ...
            return
        # Phase 33 — Deprecation gate. Construct the canonical sleeve_id
        # for this (symbol, tf, strategy_mode[, hedge_policy]) — mirrors
        # the construction in _audit (lines 4137-4155) — and skip the
        # entire dispatch when it's in TV_POLY_DEPRECATED_SLEEVES. No
        # compute, no audit row. The _f7 suffix is NOT applied here
        # because the deprecation set lists base sleeve_ids (operator
        # deprecates the strategy, not a filter variant).
        #
        # Phase 34 — Shadow controllers carry their own sleeve_id via
        # ``_audit_sleeve_id_override`` (e.g., poly_updown_sol_5m_sniper_hod_HOLD).
        # They share strategy_mode + (sym, tf) with the deprecated baseline
        # — but they're NOT the deprecated sleeve. Match the override
        # directly against the deprecated set instead of reconstructing
        # from strategy_mode (which would always match the deprecated
        # baseline and block the shadow).
        if _DEPRECATED_SLEEVE_IDS_AT_DISPATCH and self._audit_sleeve_id_override is not None:
            if self._audit_sleeve_id_override in _DEPRECATED_SLEEVE_IDS_AT_DISPATCH:
                return  # Shadow itself was explicitly deprecated
            # Shadow with non-deprecated override — skip the
            # strategy_mode-based reconstruction below.
        elif _DEPRECATED_SLEEVE_IDS_AT_DISPATCH:
            if self._inverse_kind is not None:
                _dep_sid = inverse_sleeve_id(
                    kind=self._inverse_kind, symbol=symbol, tf=tf
                )
            elif self.strategy_mode == "momo":
                _dep_sid = (
                    f"poly_updown_{symbol.lower()}_{tf}_momo_"
                    f"{_HEDGE_POLICY_SUFFIX[self.hedge_policy]}"
                )
            elif self.strategy_mode == "momo_v2":
                _dep_sid = (
                    f"poly_updown_{symbol.lower()}_{tf}_momo_v2_"
                    f"{_HEDGE_POLICY_SUFFIX[self.hedge_policy]}"
                )
            else:
                _dep_sid = (
                    f"poly_updown_{symbol.lower()}_{tf}_{self.strategy_mode}"
                )
            if _dep_sid in _DEPRECATED_SLEEVE_IDS_AT_DISPATCH:
                return  # Hard-disable — no compute, no audit
    ...
    async def _on_bar_close_impl(
        ...
        # 2026-05-21 — per-cell disable for momo_v2 F7 variants only.
        # CORRECTED per EXACT_BUG_MOMO_V2_QTY_COMPUTE_ASYMMETRY_2026_05_21.md
        # (retraction of TV_AGENT_FIX_MOMO_V2_BUGS report). NO CODE BUG:
        # _compute_qty_shares [0.05, 0.95] is the Phase 18.1 D-12 risk
        # gate working as designed. The catastrophic 5%/18% WR on eth_5m_f7
        # + btc_15m_f7 is a REGIME ARTIFACT: in strongly-trending periods
        # qty_compute correctly rejects the expensive side (best_ask>0.95),
        # leaving only cheap-side retracement fires that lose when trend
        # resumes. This deploy-level disable narrows scope to ONLY the 6
        # affected F7 sleeves (3 policies × 2 cells). Baseline non-F7
        # variants are NOT in scope — they pass through unchanged.
        # Env: TV_POLY_MOMO_V2_DISABLED_CELLS=eth_5m,btc_15m
        if (
            self.strategy_mode == "momo_v2"
            and self._f7_suffix == "_f7"
            # Phase 34 — shadow controllers test new gate stacks on top
            # of the bugged cells: the HoD filter is EXACTLY the kind of
            # regime-artifact mitigation that the disable was a placeholder
            # for. Let shadows run so we can A/B whether HoD fixes
            # eth_5m + btc_15m. Baselines stay disabled.
            and self._audit_sleeve_id_override is None
        ):
            import os as _os
            ...
            if _disabled_raw:
                ...
                if _cell_token in _disabled_cells:
                    logger.warning(
                        "poly_updown.momo_v2_f7_cell_disabled cell=%s",
                        _cell_token,
                    )
                    return
        ...
        # Phase 34 — shadow gated sleeve stack (HoD / MTF2 / Markov).
        # Runs AFTER strategy.signal() AFTER F7 (which the strategy enforces
        # internally for momo/momo_v2 cells) BUT BEFORE qty_compute / token
        # resolution / order placement. Baseline controllers (empty
        # gate_stack) skip this block — preserves byte-identical behavior
        # for the existing 35 sleeves.
        if self._gate_stack:
            ...
            _fire_unix_s = int(time.time())
            _cell = f"{symbol.lower()}_{tf}"
            _cell_key = (self._gate_cell_strategy or "", _cell)
            gate_decisions: dict[str, dict[str, Any]] = {}

            for _gate in self._gate_stack:
                if _gate == "hod":
                    _allowed = HOD_TOP8_BY_CELL.get(_cell_key, ())
                    _label = decision_label_hod(signal, _fire_unix_s, _allowed)
                    gate_decisions["hod"] = {
                        "pass": _label == "pass",
                        "hour": datetime.fromtimestamp(_fire_unix_s, UTC).hour,
                        "allowed": list(_allowed),
                    }
                    if not hod_passes(_fire_unix_s, _allowed):
                        await self._audit(
                            symbol,
                            tf,
                    ...
                    # Read from BarContext (populated by t_plus_60 / t_plus_120
                    # builders when at least one shadow sleeve in the cell
                    # needs mtf2). Fall through to NaN when BarContext is
                    # bar_close or builder skipped the fetch — gate fails closed.
                    _bctx = self._bar_ctx_active
                    _r15 = float(
                        getattr(_bctx, "ret_15m_for_mtf", None) or float("nan")
                    )
                    _r1h = float(
                        getattr(_bctx, "ret_1h_for_mtf", None) or float("nan")
                    )
                    _label = decision_label_mtf2(signal, _r15, _r1h)
                    gate_decisions["mtf2"] = {
                        "pass": _label == "pass",
                        "ret_15m": _r15 if math.isfinite(_r15) else None,
                        "ret_1h": _r1h if math.isfinite(_r1h) else None,
                    }
                    if not mtf2_passes(signal, _r15, _r1h):
                        await self._audit(
                            symbol,
                            tf,
                    ...
                    # Phase 34.1 — 1-MIN vol-adaptive Markov gate. Populated
                    # by build_bar_context_t_plus_120 (the only phase momo v1
                    # fires on). Same encoding as m5va. Same audit reason
                    # string ("gate_markov_skip") — the regime variant is
                    # disambiguated by gate_decisions key.
                    _bctx = self._bar_ctx_active
                    ...
                    # Unknown gate — fail-open per spec §3.1. Won't happen
                    # in practice because ctor validates _VALID_GATES, but
                    # the defense-in-depth log catches config drift.
                    logger.warning(
                        "poly_updown.unknown_gate",
                        extra={
                            "gate": _gate,
                            "sleeve_id": self._audit_sleeve_id_override,
                        },
                    )
            # All gates passed — stash decisions on instance so _audit can
            # attach the full record to the order_placed row. Cleared on
            # next on_bar_close invocation via the ContextVar reset path.
            self._gate_decisions_active = gate_decisions
    ...
    async def _compute_qty_shares(self, token_id: int) -> Decimal | None:
        ...
        # Phase 36 — strategy can override the per-fire notional via
        # ``_kelly_notional_override_usd`` (Kelly ensemble sleeve #1 sets
        # this in signal() based on fair_edge_bp tier). Cleared after each
        # dispatch by the controller. Hard-capped at LIVE_MIRROR_MAX_NOTIONAL
        # for live mirrors via the existing live_mirror_notional kwarg path.
        _override = getattr(self, "_kelly_notional_override_usd", None)
        effective_notional = (
            _override if _override is not None else self.notional_usd
        )
        qty = (effective_notional / best_ask).quantize(Decimal("0.0001"), rounding=ROUND_DOWN)
        if qty <= 0:
            return None
        return qty
    ...
    async def _audit(
        ...
        *,
        ...
        # Phase 34 — shadow gated sleeve audit enrichment. When passed,
        # the per-gate pass/fail record gets attached to the audit payload
        # so downstream analytics can reconstruct what each gate did. The
        # gate_stack field on the controller carries the configured stack;
        # the gate_decisions kwarg here carries the *runtime* result.
        ...
        # Phase 34 — shadow gated controllers carry the full sleeve_id
        # baked at registration time (e.g., ``poly_updown_sol_5m_sniper_hod_HOLD``).
        # When the override is set, bypass the base/momo/momo_v2/inverse
        # construction entirely — each shadow controller writes to its
        # own row stream.
        if self._audit_sleeve_id_override is not None:
            sleeve_id = self._audit_sleeve_id_override
        elif self._inverse_kind is not None:
        ...
        # Phase 33 — Last-mile deprecation gate at the audit write point.
        # The on_bar_close gate at top of dispatch is the primary skip; this
        # gate strips the _f7 suffix and compares the BASE form against the
        # deprecated set so any audit-write path (signal, hedge_skip,
        # resolution, etc.) is no-op'd when the operator has deprecated
        # this sleeve. No event row, no dashboard noise, no resolver work.
        if _DEPRECATED_SLEEVE_IDS_AT_DISPATCH:
            _base_for_dep = (
                sleeve_id[: -len(self._f7_suffix)]
                if self._f7_suffix and sleeve_id.endswith(self._f7_suffix)
                else sleeve_id
            )
            if _base_for_dep in _DEPRECATED_SLEEVE_IDS_AT_DISPATCH:
                logger.info(
                    "poly_updown.deprecation_gate_skipped",
                    sleeve_id=sleeve_id,
                    base=_base_for_dep,
                    reason=reason,
                )
        ...
        # Phase 34 — shadow gated sleeve audit enrichment. Always attach
        # gate_stack so analytics can identify which gates the controller
        # was configured with even on rows that never reached the gate
        # block (no_signal, market_not_discovered, etc.). gate_decisions
        # is the runtime record from the most recent gate evaluation —
        # caller passes it explicitly for gate_*_skip rows; for
        # order_placed rows we pull it off self (set by on_bar_close gate
        # stack on the same dispatch).
        if self._gate_stack:
            payload["gate_stack"] = list(self._gate_stack)
            _decisions = gate_decisions or self._gate_decisions_active
            if _decisions is not None:
                payload["gate_decisions"] = _decisions
        # Phase 36 — Kelly ensemble + pre-window audit enrichment.
        # Attach fire_offset_s + all Phase 36 features + Kelly metadata
        # so analytics can reproduce the fire decision.
        if self.strategy_mode in (
            "vwap_kelly_ensemble", "prewindow_s3", "prewindow_s4_15m"
        ):
            _bctx_p36 = self._bar_ctx_active
            if _bctx_p36 is not None:
                payload["entry_phase"] = getattr(_bctx_p36, "phase", None)
                payload["vwap_dev_bps"] = getattr(_bctx_p36, "vwap_dev_bps", None)
                payload["fair_edge_bp"] = getattr(_bctx_p36, "fair_edge_bp", None)
                payload["fair_up"] = getattr(_bctx_p36, "fair_up_value", None)
                payload["cvd_30s"] = getattr(_bctx_p36, "cvd_30s", None)
                payload["cvd_60s"] = getattr(_bctx_p36, "cvd_60s", None)
                payload["macd_hist"] = getattr(_bctx_p36, "macd_hist_value", None)
                payload["rvol_30_300"] = getattr(_bctx_p36, "rvol_30_300", None)
                payload["s_now"] = getattr(_bctx_p36, "s_now", None)
                payload["strike"] = getattr(_bctx_p36, "strike", None)
                payload["tau_s"] = getattr(_bctx_p36, "tau_s", None)
            if self.strategy_mode == "vwap_kelly_ensemble":
                payload["kelly_mult"] = getattr(self, "_kelly_mult", None)
                payload["kelly_rule"] = getattr(self, "_kelly_rule_label", None)
                _ov = getattr(self, "_kelly_notional_override_usd", None)
                payload["kelly_notional_usd"] = (
                    str(_ov) if _ov is not None else None
                )

        # Phase 35 — VWAP continuation audit enrichment. Attach the
        # late-fire offset + dev_bps + cross-asset devs + regime so
        # analytics can reproduce the fire decision from event rows
        # alone. Strategy itself enforces the gates (no separate
        # gate_decisions block); the raw inputs land here for forensics.
        if self.strategy_mode == "vwap_continuation":
            _bctx_vw = self._bar_ctx_active
            payload["fire_offset_s"] = self.vwap_offset_s
            if _bctx_vw is not None:
                payload["entry_phase"] = getattr(_bctx_vw, "phase", None)
                payload["vwap_dev_bps"] = getattr(_bctx_vw, "vwap_dev_bps", None)
                payload["vwap_15m_anchored"] = getattr(_bctx_vw, "vwap_15m_anchored", None)
                payload["markov_regime_w20_1m_va"] = getattr(
                    _bctx_vw, "markov_regime_w20_1m_va", None
                )
                payload["rsi_14_for_signal"] = getattr(
                    _bctx_vw, "rsi_14_for_signal", None
                )
                _cross = getattr(_bctx_vw, "cross_asset_devs", None)
                if _cross is not None:
                    payload["cross_asset_devs"] = [
                        [a, d] for (a, d) in _cross
                    ]
        ...
        # 2026-05-22 — Polymarket V2 fee formula per
        # https://docs.polymarket.com/trading/fees + /builders/fees:
        #
        #     fee_usd = C × feeRate × p × (1 - p)
        #
        # where C = shares filled and p = price. Crypto markets (UpDown is
        # crypto-binary): feeRate = 0.07. Geopolitical/world events: free.
        # Makers pay 0; only takers pay. Rebate is funded from taker fees
        # and accrued daily (NOT included on the fill receipt).
        #
        # Order of preference for the audit fee_usd field:
        #   1. Venue-reported fee (raw["fee"]) — authoritative if V2 starts
        #      surfacing it on fill receipts (currently always 0 — feeRateBps
        #      was removed from V2 Order struct, fees are protocol-side).
        #   2. Formula computation from shares + price.
        #   3. Legacy flat baseline (TV_POLY_FEE_USD_PER_FILL, default 0)
        #      — only fires when shares+price both missing (V2 never).
        #
        # Pre-fix: a flat $0.50/fill baseline produced 5000-bps phantom TCA
        # on $1-stake fills. Real fee at $1 stake / p=0.50 = 1*0.07*0.5*0.5
        # = $0.0175 (1.75% — matches the docs bell curve).
        if tx_hash:
        ...
        if fill_status == "filled":
            try:
                ...
                _venue_fee = 0.0
            if _venue_fee > 0:
                payload["fee_usd"] = _venue_fee
            else:
                # Formula path — compute from the actual fill numbers.
                _shares = Decimal("0")
                _price = Decimal("0")
                if fill_qty:
                    with contextlib.suppress(InvalidOperation, ValueError, TypeError):
                        _shares = Decimal(str(fill_qty))
                if fill_price:
                    with contextlib.suppress(InvalidOperation, ValueError, TypeError):
                        _price = Decimal(str(fill_price))
                if _shares > 0 and Decimal("0") < _price < Decimal("1"):
                    from backend.app.venues.polymarket.fees import (
                        compute_taker_fee_usd,
                    )
                    fee_decimal = compute_taker_fee_usd(
                        shares=_shares,
                        price=_price,
                        category=_TV_POLY_MARKET_CATEGORY,
                    )
                    payload["fee_usd"] = float(fee_decimal)
                else:
                    # Degenerate fill (no price or no qty parsed) — fall back
                    # to flat baseline so TCA pipelines don't choke on None.
                    payload["fee_usd"] = _TV_POLY_FEE_USD_PER_FILL

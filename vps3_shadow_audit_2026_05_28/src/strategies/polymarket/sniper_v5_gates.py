...
def g_sms_no_liquidity_above(
    ...
    **_kw: Any,
    ...
    if row is None:
        return False
    if direction == "UP":
        return row.sms_liquidity_count_above == 0
    return row.sms_liquidity_count_below == 0
...
#   * ``asset_trades`` — pre-loaded canonical trades_polymarket/{asset}.parquet
#     filtered to the sleeve's asset (one DataFrame per asset). Columns the
#     B-gates read: slug, timestamp_us, outcome ('Up'/'Down'), side ('buy'/'sell'),
#     size.
...
#     Columns the A2 gate reads: coin, time_exchange_us, notional.
#
# All gates return False on missing/empty inputs (defensive — engine boot
# succeeds even if the canonical parquets haven't been written yet; gates
# simply never fire until the operator refreshes them per CLAUDE.md inv #13
# data-staleness caveat).
...
def g_b1_poly_flow_aligned(
    ...
    **_kw: Any,
    ...
    against engine boot before the canonical parquets are loaded.
    """
...
def g_a2_hl_short_cascade(
    ...
    **_kw: Any,
    ...
    Asset coverage (per spec §2.4):
      * BTC: well-populated, $100k threshold gives WR=95.7% (n=140, t=7.5).
      * SOL/ETH: insufficient HL data, do not use.

    Defensive: returns False on missing data so V9_01/V9_02 silently skip
    until the canonical hyperliquid_liquidations_full.parquet is loaded.
    """

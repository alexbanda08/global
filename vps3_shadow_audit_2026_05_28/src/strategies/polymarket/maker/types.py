...
Action = Literal[
    "POST_BID", "POST_ASK", "CANCEL", "MERGE", "MINT", "REDEEM", "TAKE",
    # E1 (ENGINE_BUG_MAP_2026_05_28): settle/expire row emitted by the sim at
    # slot resolution to clear the loser-side mark (zero both inventories).
    "EXPIRE",
]
...
class SlugState:
    ...
    # Phase B — convergence-cancel: derived at slug-active = slug_active_ts_us
    # + window_s (300 for 5m, 900 for 15m). Strategies gate posts/cancels in
    # the final offset_s before this boundary. None for legacy paths not yet
    # populated; convergence gate is a no-op in that case (backward compat).
    slug_end_ts_us: int | None = None
    ...
    cash_recovered: Decimal = Decimal(0)     # USDC from MERGE + REDEEM market mechanisms (Bug 2 — TV_AGENT guide)
    ...
    # 2026-05-22 — Bug 9 mirror (engine side). Once the market resolves and
    # _observe_redeem fires for this slug, residual on the LOSER side is
    # provably worth $0 (CTF pays $1 only to the winning outcome). Mark-to-
    # market must drop from $0.50 → $0 to avoid overstating PnL in the CSV
    # row written by shadow_log. The simulator flips this True before the
    # REDEEM row is emitted so the same row already reflects the corrected
    # mark. Backwards compatible: pre-fix slug_states default False, so
    # pre-REDEEM behavior is unchanged.
    redeem_fired: bool = False

...
class AccPCStrategy(AccMStrategy):
    ...
    def _pc_taker_decision(
        ...
        Phase B convergence-window: no NEW PC takers within
        ``tv_poly_maker_acc_pc_stop_posting_offset_s`` of slug end.
        """
        cfg = self.config

        # === Convergence-window block — no NEW pc-taker fires ===
...
class AccPCV2Strategy(AccPCStrategy):
    """Phase C ACC-PC-V2 — adds convergence-cancel to V1 ACC-PC. No other
    behavior diff. Cells extended to ``btc_15m,eth_15m`` via
    ``tv_poly_maker_acc_pc_v2_cells``.

    Spec: ``global/strategy_lab/reports/TV_AGENT_SPEC_NEW_SLEEVES_V2_2026_05_27.md`` §2.3 + §2.5.
    """
...
__all__ = ["AccPCStrategy", "AccPCV2Strategy"]

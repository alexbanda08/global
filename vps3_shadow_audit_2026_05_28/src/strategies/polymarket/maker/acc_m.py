...
class AccMStrategy(MakerStrategyBase):
    ...
    def on_l25_update(self, evt: L25Update) -> list[Decision]:
        ...
        if up_cached is not None and dn_cached is not None:
            ...
            #
            # Phase C — V2 sleeves use ``tv_poly_maker_acc_m_v2_enable_pat``
            # (default False — PAT is the structural bleed source per
            # TV_AGENT_SPEC_NEW_SLEEVES_V2_2026_05_27.md §2.1). ACC-H-V2 /
            # ACC-PC-V2 inherit V1 behavior since they don't override PAT.
            if self.variant == "v2":
                _pat_enabled = bool(
                    getattr(self.config, "tv_poly_maker_acc_m_v2_enable_pat", False)
                )
            else:
                _pat_enabled = bool(
                    getattr(self.config, "tv_poly_maker_acc_m_enable_pat", False)
                )
            if _pat_enabled:
                decisions.extend(
    ...
    def on_slug_resolved(self, evt: SlugResolved) -> list[Decision]:
        ...
        # TV_AGENT residual-mark fix: zero BOTH sides post-resolution.
        # Pre-fix, only the WINNER side was zeroed (by sim._observe_redeem when
        # the cascade ran). The LOSER side retained its mint quantity but is
        # provably $0 per CTF binary protocol. shadow_log._row_from_decision
        # marks residual at $0.50/share — turning $0 worthless shares into
        # phantom $15 unrealized gain on every RESOLVED MAS slug. Operator
        # complaint: "MAS at +$485, computing the redeem as profit, but its
        # not". Also covers the loser-only inv case (no REDEEM emitted because
        # winner_inv=0; loser inv was inflating the mark with no compensation).
        # Sim's _observe_redeem uses max(0, x-N) clamp so this won't cause
        # double-decrement when the REDEEM Decision flows through observe().
        # cash_recovered is credited by sim, not here (per TV_AGENT spec note).
    ...
    def _cancel_decisions(
        ...
        Rule 0 (convergence-window) — within stop_posting_offset_s of slug end,
        force-cancel ALL open orders on BOTH sides + bypass per-rule eval. The
        single-rule offset key is ``tv_poly_maker_{code}_stop_posting_offset_s``
        (per Phase B spec §2.3). See ``TV_AGENT_FIX_CONVERGENCE_CANCEL_SPEC.md``.
    ...
    def _post_decisions(
        ...
        Phase B (convergence-window) — within
        ``tv_poly_maker_{code}_stop_posting_offset_s`` seconds of slug end,
        return [] immediately so no NEW bids land in the high-adverse-selection
        tail. See ``TV_AGENT_FIX_CONVERGENCE_CANCEL_SPEC.md`` §2.2.
        """
        decisions: list[Decision] = []
        ...
        #
        # Phase C — ACC-M-V2 reads its own knob set
        # (TV_AGENT_SPEC_NEW_SLEEVES_V2_2026_05_27.md §2.1). ACC-H-V2 /
        # ACC-PC-V2 fall through to the ACC-M V1 keys since their spec only
        # adds convergence-cancel (§2.2 / §2.3 — no _post_decisions tuning).
        if self.code == "ACC-M-V2":
            post_size = Decimal(self.config.tv_poly_maker_acc_m_v2_post_size or 0) or \
                self.config.tv_poly_maker_min_post_shares
            max_imbalance = Decimal(
                self.config.tv_poly_maker_acc_m_v2_max_imbalance_shares or 0
            ) or _MAX_IMBALANCE_SHARES
            abs_max = Decimal(
                self.config.tv_poly_maker_acc_m_v2_absolute_max_inventory or 0
    ...
    def _pat_decisions(
        ...
        Phase B convergence-window: no NEW takes within
        ``tv_poly_maker_{code}_stop_posting_offset_s`` of slug end (see
        ``TV_AGENT_FIX_CONVERGENCE_CANCEL_SPEC.md`` §2.4).
        """
        cfg = self.config

        # === Convergence-window block — no NEW takes ===
        if state.slug_end_ts_us is not None:
            offset_s = int(
                getattr(
                    cfg,
...
class AccMV2Strategy(AccMStrategy):
    """Phase C ACC-M-V2 — disables the PAT taker overlay (V1's structural
    bleed source — -$2,983/day from PAT-SHADOW + -$446/day from ACC-M PAT)
    and reads its own POST_SIZE / MAX_IMBALANCE / ABS_MAX knob set so the
    A/B vs V1 stays clean. Convergence-cancel inherits from AccMStrategy
    via ``tv_poly_maker_acc_m_v2_stop_posting_offset_s``.

    Spec: ``global/strategy_lab/reports/TV_AGENT_SPEC_NEW_SLEEVES_V2_2026_05_27.md`` §2.1.
    """
...
__all__ = ["AccMStrategy", "AccMV2Strategy"]

...
def introspect_sleeves(
    *,
    ...
    # 2026-05-27 — TV_POLY_DEPRECATED_HIDE wipe-mode toggle. When set to
    # "true", deprecated sleeves are NOT re-added to the manifest — they
    # disappear from the dashboard entirely instead of rendering as
    # opacity-60 cards in the (currently non-existent) Deprecated section.
    # VPS3 sets this true so 12 retired sleeve_ids (sniper variants +
    # v3/v3_2/v3_3/v4) stop cluttering the operator view. Default false
    # preserves the Phase 33 grayed-card behavior.
    import os as _os  # noqa: PLC0415 — local import, env read on each call
    _hide_deprecated = (
        _os.environ.get("TV_POLY_DEPRECATED_HIDE", "false").strip().lower() == "true"
    )

    # Phase 33 (D-04) — Manifest unions active poly_sleeve_ids with the
    # deprecated set so the dashboard can render the Deprecated section.
    # Deprecated sleeves are pre-filtered OUT of poly_sleeve_ids by
    # bots._resolve_poly_updown_sleeve_ids() (D-05), so we re-add them
    # here. Dedup while preserving order.
    #
    # When _hide_deprecated is true, we skip the re-add — deprecated
    # sleeves are gone from the manifest entirely (no card at all).
    poly_sleeve_ids_all: tuple[str, ...]
    if _DEPRECATED_POLY_UPDOWN_SLEEVE_IDS and not _hide_deprecated:

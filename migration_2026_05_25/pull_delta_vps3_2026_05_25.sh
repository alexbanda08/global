#!/bin/bash
# Pull delta 2026-05-21 00:00 UTC -> NOW from VPS3 for all canonical sources.
# Overlaps with last refresh_2026_05_21 (which had data through ~May 21 20:10 UTC) by ~20h for safety.
# NOTE: T_START_US=1779321600000000 is 2026-05-21 00:00 UTC, NOT May 20 (verified
# 2026-05-25 against Python datetime: epoch 1779321600 = 2026-05-21T00:00:00Z).
#
...
# Window: 2026-05-21 00:00 UTC (overlap with last delta for safety)
T_START_US=1779321600000000          # 2026-05-21 00:00 UTC
TAG=2026_05_25
echo "==============================================="
...
for TAB in hyperliquid_klines_v2 hyperliquid_trades_v2 hyperliquid_liquidations_v2; do
  ...
  # Both trades and liquidations use time_exchange_us (verified 2026-05-25 via \d).
  # Earlier "timestamp_us" was a column-name bug that silently produced empty
  # delta files — see HANDOFF_2026_05_25_DATA_REFRESH if reviewing.
  ...
  T0=$(date +%s)
  sudo -u postgres psql -d storedata -v ON_ERROR_STOP=1 -c "\copy (SELECT * FROM ${TAB} WHERE ${TS_COL} >= ${T_START_US} ORDER BY ${TS_COL}) TO '${OUT}' CSV HEADER" 2>/dev/null || echo "  ($TAB schema mismatch — skipping)"
  [[ -f $OUT ]] && { echo "  rows: $(wc -l < $OUT)"; gzip -f $OUT; }
done

# 9. binance metrics (perp OI, LS ratio) — INTENTIONALLY SKIPPED
# Binance futures API is GEO-BLOCKED on VPS3 (Ireland). The binance_metrics_v2
# table on VPS3 only contains spot-derived rows or is empty, and the schema
# never matches what we'd want for futures OI / long-short ratio. Do NOT
# re-enable this block on VPS3. If perp metrics are needed in the future,
# pull from a non-blocked VPS via the storedata agent.
